#!/usr/bin/env python3
"""
mini_designer.py — 工业GUI设计器 v20
功能: 控件树拖拽/撤销重做/批量移动/预览/图表/仪表/数据模拟/智能等间距吸附
核心修复: P14内置曲线控件(消除花屏) + Smart Snap Guides(等间距/容器居中)
"""
import sys, os, re, json, math, random, importlib.util
from collections import defaultdict, deque
from abc import ABC, abstractmethod

# 屏蔽 Qt 的 QPainter 警告（显卡兼容性问题）
...
from PySide6.QtGui import QDrag, QColor, QFont, QPainter, QPen, QKeySequence, QShortcut, QBrush, QFontMetrics, QPainterPath
...
# ══════════════════════════════════════════════════════════════
# P14: 实时曲线控件 (CurveWidget) - 内置版，无需外部依赖
# ══════════════════════════════════════════════════════════════
class CurveWidget(QWidget):
    """工业级实时趋势曲线 — 纯QPainter渲染，修复设计器花屏/重叠问题"""
    def __init__(self, parent=None):
        ...
        self._x_range = 60
        self._y_min = 0.0; self._y_max = 100.0
    ...
    def setChannelCount(self, n):
        ...
        while len(self._channels) < n:
            ...
            self._channels.append({"name": f"通道{idx + 1}",
    ...
    def setRange(self, y_min, y_max):
        ...
        self._auto_range = False; self.update()
    ...
    def paintEvent(self, e):
        ...
        if self._auto_range:
            ...
            for ch in self._channels:
                if ch["data"]: all_vals.extend(ch["data"])
            if all_vals:
                self._y_min = min(all_vals) - 5
                self._y_max = max(all_vals) + 5
                if abs(self._y_max - self._y_min) < 1:
                    center = (self._y_max + self._y_min) / 2
                    self._y_min = center - 10
                    self._y_max = center + 10
        val_range = self._y_max - self._y_min
        if val_range <= 0: val_range = 1
        for ch in self._channels:
            data = list(ch["data"])
            if not data: continue
            path = QPainterPath()
            step_x = pw / max(1, len(data) - 1)
            for i, v in enumerate(data):
                x = margin_l + i * step_x
                normalized = (v - self._y_min) / val_range
                y = margin_t + ph * (1.0 - normalized)
        ...
        p.drawText(margin_l, margin_t - 8, self._title)
        legend_y = h - 15
        lx = margin_l
        for ch in self._channels:
            p.setPen(ch["color"])
            p.drawLine(lx, legend_y, lx + 20, legend_y)
            p.setPen(QColor("#666"))
            p.setFont(QFont("Microsoft YaHei", 8))
            p.drawText(lx + 24, legend_y + 3, ch["name"])
            lx += 70
        p.end()
    def sizeHint(self): return QSize(360, 200)
    def minimumSizeHint(self): return QSize(100, 80)

CUSTOM_WIDGETS = []
def get_all_categories():
    cats = list(BUILTIN_CATEGORIES)
    cats.insert(4, ("📈 工业图表", [("实时趋势曲线", CurveWidget, {})]))
    if CUSTOM_WIDGETS:
    ...
    try:
        ...
        if svg_items: cats.append(("🎨 SVG 图元库", svg_items))
...
def get_display_to_entry():
    ...
    mapping["实时趋势曲线"] = ("实时趋势曲线", CurveWidget, {})
...
def _save_user_templates():
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "user_templates.json")
    try:
        with open(path, "w", encoding="utf-8") as f: json.dump(USER_TEMPLATES, f, ensure_ascii=False, indent=2)
    except: pass
...
class QssBuilderDialog(QDialog):
    WIDGET_STYLE_MAP = {
        "QPushButton": ["bgColor", "borderRadius", "borderWidth", "borderColor", "fontSize", "fontColor", "hoverBg", "padding"],
        "QToolButton": ["bgColor", "borderRadius", "borderWidth", "borderColor", "fontSize", "fontColor", "hoverBg", "padding"],
        "QLineEdit": ["bgColor", "borderRadius", "borderWidth", "borderColor", "fontSize", "fontColor", "focusBorder", "padding"],
        "QComboBox": ["bgColor", "borderRadius", "borderWidth", "borderColor", "fontSize", "fontColor", "padding"],
        "QLabel": ["fontSize", "fontColor", "fontWeight", "bgColor"],
        "QGroupBox": ["bgColor", "borderRadius", "borderWidth", "borderColor", "titleColor", "padding"],
        "QProgressBar": ["bgColor", "borderRadius", "chunkColor", "height"],
        "QSlider": ["grooveColor", "handleColor", "handleSize"],
        "default": ["bgColor", "borderRadius", "borderWidth", "borderColor", "fontSize", "fontColor"]
    }
    def __init__(self, widget, parent=None):
        ...
        self.preview_container.setStyleSheet("QFrame{background:#f5f5f5; border:1px dashed #ccc; border-radius:8px;}")
        preview_layout = QVBoxLayout(self.preview_container); preview_layout.setAlignment(Qt.AlignCenter)
        ...
        self.code_display.setReadOnly(True); self.code_display.setMaximumHeight(120)
        self.code_display.setStyleSheet("QPlainTextEdit{font-family:'Consolas',monospace; font-size:11px; background:#1e1e1e; color:#d4d4d4; border:1px solid #333; border-radius:4px; padding:6px;}")
        ...
        self.form_layout.setSpacing(8); self.form_layout.setContentsMargins(4, 4, 4, 4)
        self.editors = {}
        scroll.setWidget(self.form_widget); control_panel.addWidget(scroll)
        btn_layout = QHBoxLayout()
        btn_reset = QPushButton("🔄 重置"); btn_reset.clicked.connect(self._reset_to_default)
        btn_apply = QPushButton("✅ 应用")
        btn_apply.setStyleSheet("QPushButton{background:#27AE60;color:#fff;border:none;border-radius:4px;padding:6px 18px;font-weight:bold;} QPushButton:hover{background:#219A52;}")
        btn_apply.clicked.connect(self.accept)
        btn_cancel = QPushButton("❌ 取消"); btn_cancel.clicked.connect(self.reject)
        btn_layout.addWidget(btn_reset); btn_layout.addStretch(); btn_layout.addWidget(btn_apply); btn_layout.addWidget(btn_cancel)
        control_panel.addLayout(btn_layout)
        layout.addLayout(preview_panel, 1); layout.addLayout(control_panel, 1)
    def _create_preview_widget(self):
        cls = type(self.target_widget)
        try:
            w = cls(); w.setMinimumSize(80, 30)
            if hasattr(w, 'setText') and hasattr(self.target_widget, 'text'): w.setText(self.target_widget.text() or "预览文本")
            if hasattr(w, 'setTitle') and hasattr(self.target_widget, 'title'): w.setTitle(self.target_widget.title() or "预览标题")
            if isinstance(w, QProgressBar): w.setValue(65)
            if isinstance(w, QSlider): w.setValue(60)
            if isinstance(w, QComboBox): w.addItems(["选项1", "选项2"]); w.setMinimumWidth(160)
            return w
        except Exception:
            fallback = QLabel("预览不可用"); fallback.setAlignment(Qt.AlignCenter); fallback.setMinimumSize(80, 30)
            return fallback
    def _build_editors(self):
        supported = self.WIDGET_STYLE_MAP.get(self.widget_class, self.WIDGET_STYLE_MAP["default"])
        editor_configs = {
        ...
        for prop in supported:
            if prop not in editor_configs: continue
            cfg = editor_configs[prop]; label_text = cfg[0]; editor_type = cfg[1]; default_val = cfg[2]
            if editor_type == "color":
                editor = QPushButton(); editor.setFixedSize(80, 28)
                editor.setStyleSheet(f"QPushButton{{background:{default_val}; border:1px solid #aaa; border-radius:4px;}}")
                ...
                editor = QSpinBox(); editor.setRange(*cfg[3]); editor.setValue(int(default_val))
                editor.valueChanged.connect(lambda v, p=prop: self._on_value_changed(p, v))
            elif editor_type == "check":
                editor = QCheckBox(); editor.setChecked(bool(default_val))
                editor.stateChanged.connect(lambda s, p=prop: self._on_value_changed(p, bool(s)))
            else: continue
            self.editors[prop] = editor; self.form_layout.addRow(label_text + ":", editor)
    def _pick_color(self, btn, prop):
        current = btn.property("_color")
        color = QColorDialog.getColor(QColor(current), self, f"选择{prop}颜色")
        if color.isValid():
            hex_color = color.name(); btn.setProperty("_color", hex_color)
            btn.setStyleSheet(f"QPushButton{{background:{hex_color}; border:1px solid #aaa; border-radius:4px;}}")
            self._on_value_changed(prop, hex_color)
    def _on_value_changed(self, prop, value):
        self.current_style[prop] = value; self._update_preview()
    def _update_preview(self):
        if self.preview_widget is None: return
        qss = self._generate_qss(); self.preview_widget.setStyleSheet(qss); self.code_display.setPlainText(qss)
    def _generate_qss(self):
        s = self.current_style; cls = self.widget_class; lines = []; props = []
        if "bgColor" in s: props.append(f"background:{s['bgColor']};")
        if "borderRadius" in s: props.append(f"border-radius:{s['borderRadius']}px;")
        if "borderWidth" in s and "borderColor" in s: props.append(f"border:{s['borderWidth']}px solid {s['borderColor']};")
        ...
        if props: lines.append(f"{cls} {{ {' '.join(props)} }}")
        if "hoverBg" in s:
            hover_props = [f"background:{s['hoverBg']};"]
            if "borderColor" in s: hover_props.append(f"border-color:{s['borderColor']};")
            lines.append(f"{cls}:hover {{ {' '.join(hover_props)} }}")
        if "focusBorder" in s: lines.append(f"{cls}:focus {{ border-color:{s['focusBorder']}; }}")
        if cls == "QGroupBox" and "titleColor" in s: lines.append(f"{cls}::title {{ color:{s['titleColor']}; }}")
        if cls == "QProgressBar":
            if "chunkColor" in s:
                cr = s.get("borderRadius", 3)
                lines.append(f"{cls}::chunk {{ background:{s['chunkColor']}; border-radius:{cr}px; }}")
            if "height" in s: lines.append(f"{cls} {{ min-height:{s['height']}px; }}")
        if cls == "QSlider":
            if "grooveColor" in s: lines.append(f"{cls}::groove:horizontal {{ background:{s['grooveColor']}; height:6px; border-radius:3px; }}")
            if "handleColor" in s:
                hs = s.get("handleSize", 16); margin = -(hs // 2 - 3)
                lines.append(f"{cls}::handle:horizontal {{ width:{hs}px; height:{hs}px; margin:{margin}px 0; background:{s['handleColor']}; border-radius:{hs//2}px; }}")
        return "\n".join(lines)
    def _parse_existing_qss(self, qss_str):
        result = {}
        if not qss_str: return result
        import re
        patterns = {"bgColor": r"background\s*:\s*(#[0-9a-fA-F]{3,8})", "borderRadius": r"border-radius\s*:\s*(\d+)px",
                    "borderWidth": r"border\s*:\s*(\d+)px", "borderColor": r"border\s*:\s*\d+px\s+solid\s+(#[0-9a-fA-F]{3,8})",
                    "fontSize": r"font-size\s*:\s*(\d+)p[tx]", "fontColor": r"(?<!-)color\s*:\s*(#[0-9a-fA-F]{3,8})",
                    "fontWeight": r"font-weight\s*:\s*bold", "padding": r"padding\s*:\s*(\d+)px"}
    ...
    def _reset_to_default(self):
        ...
        for prop, editor in self.editors.items():
            if isinstance(editor, QPushButton):
                editor.setProperty("_color", "#ffffff"); editor.setStyleSheet("QPushButton{background:#ffffff; border:1px solid #aaa; border-radius:4px;}")
            elif isinstance(editor, QSpinBox):
                defaults = {"borderRadius": 4, "borderWidth": 1, "fontSize": 13, "padding": 6, "height": 20, "handleSize": 16}
                editor.setValue(defaults.get(prop, 0))
            elif isinstance(editor, QCheckBox): editor.setChecked(False)
        self._update_preview()
    def get_qss(self): return self._generate_qss()
    def showEvent(self, event):
        super().showEvent(event)
        if not self.editors: self._build_editors()
        if self.preview_widget is None:
            layout = self.preview_container.layout()
            if hasattr(self, '_preview_placeholder') and self._preview_placeholder:
                layout.removeWidget(self._preview_placeholder); self._preview_placeholder.deleteLater(); self._preview_placeholder = None
            self.preview_widget = self._create_preview_widget(); layout.addWidget(self.preview_widget)
        self._update_preview()

class CallbackEditorDialog(QDialog):
    def __init__(self, canvas, slot_name, existing_code="", parent=None):
        super().__init__(parent); self.canvas = canvas; self.slot_name = slot_name
        self.setWindowTitle(f"✏️ 编辑回调函数: {slot_name}"); self.resize(700, 500)
        layout = QVBoxLayout(self)
        hint = QLabel(f"正在编辑回调函数: <b>{slot_name}</b>  按 <b>Ctrl+S</b> 保存  <b>ESC</b> 取消")
        hint.setStyleSheet("color:#666; font-size:12px; padding:4px; background:#f5f5f5; border-radius:4px;")
        layout.addWidget(hint)
        self.editor = QPlainTextEdit(); self.editor.setPlainText(existing_code or self._default_code())
        self.editor.setStyleSheet("QPlainTextEdit { font-family: 'Consolas', 'Courier New', monospace; font-size: 13px; background: #1e1e1e; color: #d4d4d4; border: 1px solid #333; border-radius: 4px; padding: 8px; }")
        self.editor.setTabStopDistance(QFontMetrics(self.editor.font()).horizontalAdvance(' ') * 4)
        layout.addWidget(self.editor)
        help_text = QLabel("💡 提示: 在此编写该回调函数的完整代码。函数签名中的 <code>self</code> 即为生成的窗口实例，可通过 <code>self.btn_start</code> 等方式访问其他控件。")
        help_text.setWordWrap(True); help_text.setStyleSheet("color:#888; font-size:11px; padding:4px;")
        layout.addWidget(help_text)
        btn_layout = QHBoxLayout()
        self.btn_save = QPushButton("💾 保存并关闭")
        self.btn_save.setStyleSheet("QPushButton{background:#27AE60;color:#fff;border:none;border-radius:4px;padding:6px 18px;font-size:13px;font-weight:bold;}QPushButton:hover{background:#219A52;}")
        self.btn_cancel = QPushButton("❌ 取消")
        self.btn_cancel.setStyleSheet("QPushButton{background:#f0f0f0;border:1px solid #ccc;border-radius:4px;padding:6px 18px;font-size:13px;}")
    ...
    def _default_code(self):
        return f"def {self.slot_name}(self):\n    # TODO: 在此编写 {self.slot_name} 的回调逻辑\n    pass\n"
    def get_code(self): return self.editor.toPlainText()

class SignalSlotDialog(QDialog):
    def __init__(self, canvas, parent=None):
        super().__init__(parent); self.canvas = canvas; self.setWindowTitle("🔗 信号/槽编辑器"); self.resize(560, 500)
        layout = QVBoxLayout(self); self.conn_list = QListWidget(); layout.addWidget(self.conn_list)
        self.conn_list.setContextMenuPolicy(Qt.CustomContextMenu); self.conn_list.customContextMenuRequested.connect(self._show_conn_context_menu)
        form = QHBoxLayout()
        self.combo_source = QComboBox(); self.combo_signal = QComboBox(); self.edit_slot = QLineEdit()
        self.edit_slot.setPlaceholderText("槽函数名 (如 on_btn_start_clicked)")
        form.addWidget(QLabel("源:")); form.addWidget(self.combo_source); form.addWidget(QLabel("信号:")); form.addWidget(self.combo_signal)
        form.addWidget(QLabel("→")); form.addWidget(QLabel("槽:")); form.addWidget(self.edit_slot)
        layout.addLayout(form)
        btn_layout = QHBoxLayout()
        self.btn_add = QPushButton("➕ 添加连接"); self.btn_remove = QPushButton("➖ 删除选中")
        self.btn_edit_callback = QPushButton("✏️ 编辑回调..."); self.btn_close = QPushButton("关闭")
        btn_layout.addWidget(self.btn_add); btn_layout.addWidget(self.btn_remove); btn_layout.addWidget(self.btn_edit_callback)
        btn_layout.addStretch(); btn_layout.addWidget(self.btn_close); layout.addLayout(btn_layout)
        self.btn_add.clicked.connect(self._add_connection); self.btn_remove.clicked.connect(self._remove_connection)
        self.btn_edit_callback.clicked.connect(self._edit_selected_callback); self.btn_close.clicked.connect(self.accept)
        self.combo_source.currentTextChanged.connect(self._on_source_changed); self._refresh_lists()
    def _show_conn_context_menu(self, pos):
        item = self.conn_list.itemAt(pos)
        if not item: return
        row = self.conn_list.row(item)
        if row < 0 or row >= len(self.canvas._signal_connections): return
        menu = QMenu(self); act_edit = menu.addAction("✏️ 编辑回调函数..."); act_remove = menu.addAction("➖ 删除连接")
        chosen = menu.exec(self.conn_list.mapToGlobal(pos))
        if chosen == act_edit: self._edit_callback_at(row)
        elif chosen == act_remove: self._remove_at(row)
    def _remove_at(self, row):
        if 0 <= row < len(self.canvas._signal_connections):
            self.canvas._signal_connections.pop(row); self.canvas.widget_modified.emit(); self._refresh_lists()
    def _edit_selected_callback(self):
        row = self.conn_list.currentRow()
        if row < 0 or row >= len(self.canvas._signal_connections):
            QMessageBox.warning(self, "提示", "请先选中一个连接"); return
        self._edit_callback_at(row)
    def _edit_callback_at(self, row):
        conn = self.canvas._signal_connections[row]; slot_name = conn["slot"]
        ...
        if dlg.exec() == QDialog.Accepted:
            self.canvas._callback_code[slot_name] = dlg.get_code(); self.canvas.widget_modified.emit()
    def _get_all_widgets(self):
        result = []
        for w in self.canvas._canvas_widgets:
            if not w.property("_designer_hidden"):
                name = w.objectName() or type(w).__name__; result.append((name, w))
        return result
    def _refresh_lists(self):
        self.conn_list.clear(); widgets = self._get_all_widgets(); self.combo_source.clear()
        for name, _ in widgets: self.combo_source.addItem(name)
    ...
    def _on_source_changed(self, source_name):
        self.combo_signal.clear(); widgets = self._get_all_widgets()
        for name, w in widgets:
            if name == source_name:
                cls_name = type(w).__name__; signals = WIDGET_SIGNALS.get(cls_name, ["clicked()"])
                self.combo_signal.addItems(signals); break
    def _add_connection(self):
        source = self.combo_source.currentText(); signal = self.combo_signal.currentText(); slot = self.edit_slot.text().strip()
        if not source or not signal or not slot:
            QMessageBox.warning(self, "提示", "请填写完整的源、信号和槽函数名"); return
        conn = {"source": source, "signal": signal, "slot": slot}
        self.canvas._signal_connections.append(conn); self.canvas.widget_modified.emit(); self._refresh_lists()
...
class _DragTree(QTreeWidget):
    def __init__(self, panel, parent=None):
        super().__init__(parent); self._panel = panel; self._highlight_item = None
    def dragMoveEvent(self, event):
        ...
        if item != self._highlight_item:
            ...
            if item:
                wid = item.data(0, Qt.UserRole); w = self._panel._find_widget_by_id(wid) if wid else None
                if w and hasattr(w, '_content_layout'):
                    self._highlight_item = item; item.setBackground(0, QColor("#d0e4f7"))
    def dragLeaveEvent(self, event):
        super().dragLeaveEvent(event); self._clear_highlight()
    def dropEvent(self, event):
        self._clear_highlight(); old_state = self._panel._snapshot_tree()
        super().dropEvent(event); self._panel._sync_tree_to_canvas(old_state)
    def _clear_highlight(self):
        if self._highlight_item:
            self._highlight_item.setBackground(0, QColor(Qt.transparent)); self._highlight_item = None

class WidgetTreePanel(QWidget):
    item_selected = Signal(object)
    def __init__(self, canvas, parent=None):
        super().__init__(parent); self.canvas = canvas; self.setMinimumWidth(180); self.setMaximumWidth(300)
        ...
        self.tree = _DragTree(self); self.tree.setHeaderHidden(True); self.tree.setDragEnabled(True)
        self.tree.setAcceptDrops(True); self.tree.setDropIndicatorShown(True)
        self.tree.setDragDropMode(QAbstractItemView.InternalMove); self.tree.setDefaultDropAction(Qt.MoveAction)
        ...
        layout.addWidget(self.tree); self.tree.itemClicked.connect(self._on_item_clicked)
    ...
    def rebuild(self):
        ...
        for w in self.canvas._canvas_widgets:
            ...
            item = self._build_item(w); self.tree.addTopLevelItem(item)
            if hasattr(w, "_content_layout"): self._build_children(item, w._content_layout)
    def _build_item(self, w):
        name = w.objectName(); display = w.property("_display_name") or ""
        locked = "🔒" if w.property("_locked") else ""; tag = w.property("_tag") or ""
        tag_str = f" [{tag}]" if tag else ""; text = f"{locked}{name} ({display}{tag_str})"
        item = QTreeWidgetItem([text]); item.setData(0, Qt.UserRole, id(w))
        if w.property("_locked"): item.setForeground(0, QColor("#999"))
        return item
    def _build_children(self, parent_item, layout):
        for i in range(layout.count()):
            child = layout.itemAt(i).widget()
            if child and not child.property("_designer_hidden"):
                child_item = self._build_item(child); parent_item.addChild(child_item)
    def _find_widget_by_id(self, wid):
        for w in self.canvas._canvas_widgets:
            if id(w) == wid: return w
            if hasattr(w, "_content_layout"):
                for i in range(w._content_layout.count()):
                    cw = w._content_layout.itemAt(i).widget()
                    if cw and id(cw) == wid: return cw
        return None
    def _snapshot_tree(self):
        snap = {}
        for ri in range(self.tree.topLevelItemCount()):
            ti = self.tree.topLevelItem(ri); wid = ti.data(0, Qt.UserRole)
    ...
    def _snapshot_children(self, parent_item, snap):
        for ci in range(parent_item.childCount()):
            child = parent_item.child(ci); wid = child.data(0, Qt.UserRole)
    ...
    def _sync_tree_to_canvas(self, old_state):
        ...
        for wid, (new_parent_wid, new_idx) in new_state.items():
            ...
            if old_parent_wid == new_parent_wid and old_idx == new_idx: continue
            ...
            if old_container is None and new_container is None: self._reorder_canvas_widget(w, new_idx)
            elif old_container and new_container is None: self.canvas.history.push(ExtractWidgetCmd(self.canvas, w, old_container, old_idx))
            elif old_container is None and new_container: self._move_into_container(w, new_container, new_idx)
            elif old_container and new_container: self._move_between_containers(w, old_container, new_container, new_idx)
        self.canvas.widget_modified.emit(); self.rebuild()
    def _reorder_canvas_widget(self, w, new_idx):
        if w not in self.canvas._canvas_widgets: return
        self.canvas._canvas_widgets.remove(w); self.canvas._canvas_widgets.insert(min(new_idx, len(self.canvas._canvas_widgets)), w); w.raise_()
    ...
    def _on_item_clicked(self, item, col):
        wid = item.data(0, Qt.UserRole); w = self._find_widget_by_id(wid)
        if w: self.canvas._select(w); self.item_selected.emit(w)
    def _show_context_menu(self, pos):
        item = self.tree.itemAt(pos)
        if not item: return
        wid = item.data(0, Qt.UserRole); w = self._find_widget_by_id(wid)
        if not w: return
        menu = QMenu(self); act_select = menu.addAction("🎯 选中控件"); menu.addSeparator()
        parent = w.parent(); in_container = parent and hasattr(parent, "_content_layout")
        if in_container:
            act_up = menu.addAction("⬆️ 上移"); act_down = menu.addAction("⬇️ 下移"); menu.addSeparator()
            act_extract = menu.addAction("📤 移出容器"); menu.addSeparator()
        locked = bool(w.property("_locked")); act_lock = menu.addAction("🔓 解锁" if locked else "🔒 锁定")
        hidden = bool(w.property("_designer_hidden")); act_hide = menu.addAction("👁️ 显示" if hidden else "👁️‍🗨️ 隐藏")
        menu.addSeparator(); act_delete = menu.addAction("🗑️ 删除控件")
        chosen = menu.exec(self.tree.mapToGlobal(pos))
        if not chosen: return
        if chosen == act_select: self.canvas._select(w)
        elif in_container and chosen == act_up:
            ly = parent._content_layout; idx = ly.indexOf(w)
            if idx > 0: self.canvas.history.push(ReorderWidgetCmd(parent, w, idx, idx - 1)); self.canvas.widget_modified.emit()
        elif in_container and chosen == act_down:
            ly = parent._content_layout; idx = ly.indexOf(w)
            if idx < ly.count() - 1: self.canvas.history.push(ReorderWidgetCmd(parent, w, idx, idx + 1)); self.canvas.widget_modified.emit()
        elif in_container and chosen == act_extract:
            ly = parent._content_layout; idx = ly.indexOf(w)
            self.canvas.history.push(ExtractWidgetCmd(self.canvas, w, parent, idx)); self.canvas._deselect()
        elif chosen == act_lock: self.canvas._toggle_lock()
        elif chosen == act_hide: self.canvas._toggle_hide()
        elif chosen == act_delete: self.canvas._delete_selected()
...
class DataSimulatorPanel(QWidget):
    ...
    def __init__(self, canvas, parent=None):
        super().__init__(parent); self.canvas = canvas; self.setMinimumWidth(200); self.setMaximumWidth(400)
        layout = QVBoxLayout(self); layout.setContentsMargins(0, 0, 0, 0); layout.setSpacing(4)
        self.header = QLabel("<b>🔬 数据模拟器</b>")
        self.header.setStyleSheet("padding:4px 8px; font-size:12px; background:#f0f0f0; border-bottom:1px solid #ddd;")
        layout.addWidget(self.header)
        self.table = QTableWidget(0, 4); self.table.setHorizontalHeaderLabels(["Tag", "当前值", "模式", "参数"])
        ...
        self.btn_refresh = QPushButton("🔄 刷新"); self.btn_start = QPushButton("▶ 开始模拟"); self.btn_stop = QPushButton("⏹ 停止")
        self.btn_stop.setEnabled(False)
        for btn in [self.btn_refresh, self.btn_start, self.btn_stop]:
            btn.setStyleSheet("QPushButton{padding:4px 8px; font-size:11px;}"); btn_row.addWidget(btn)
        btn_row.addStretch(); layout.addLayout(btn_row)
        self._timer = QTimer(self); self._timer.timeout.connect(self._tick)
        self._sim_data = {}; self._elapsed = 0.0; self._running = False
        self.btn_refresh.clicked.connect(self.refresh); self.btn_start.clicked.connect(self.start); self.btn_stop.clicked.connect(self.stop)
            ...
            for w in widgets:
                ...
                if tag and (isinstance(w, BINDABLE_WIDGETS) or hasattr(w, 'setValue')):
                    if tag not in self._sim_data:
                        cur_val = 0.0
                        try:
                            if hasattr(w, "value") and callable(w.value): cur_val = float(w.value())
                            elif hasattr(w, "text") and callable(w.text):
                                try: cur_val = float(w.text())
                                except: cur_val = 0.0
                        except: pass
                        self._sim_data[tag] = {"mode": "手动", "value": cur_val, "phase": random.random() * 6.28, "min": 0.0, "max": 100.0, "period": 5.0, "widget": w}
                    if hasattr(w, "_content_layout"):
                        children = [w._content_layout.itemAt(i).widget() for i in range(w._content_layout.count()) if w._content_layout.itemAt(i) and w._content_layout.itemAt(i).widget()]
                        scan(children)
        scan(self.canvas._canvas_widgets)
        row = 0
        for tag, sd in self._sim_data.items():
            self.table.insertRow(row)
            ki = QTableWidgetItem(tag); ki.setFlags(ki.flags() & ~Qt.ItemIsEditable); ki.setBackground(QColor("#f5f5f5")); self.table.setItem(row, 0, ki)
            ...
            pi = QTableWidgetItem(param_text); pi.setToolTip("格式: min=最小值 max=最大值 T=周期(秒)"); self.table.setItem(row, 3, pi)
            row += 1
        self._busy = False
    def _on_mode_changed(self, tag, mode):
        if tag in self._sim_data: self._sim_data[tag]["mode"] = mode
    def _on_cell_changed(self, row, col):
        if self._busy or col != 3: return
        tag_item = self.table.item(row, 0); param_item = self.table.item(row, 1)
        if not tag_item: return
        tag = tag_item.text()
        if tag not in self._sim_data: return
        if col == 1 and param_item:
            try: self._sim_data[tag]["value"] = float(param_item.text())
    ...
    def start(self):
        self._running = True; self._elapsed = 0.0; self.btn_start.setEnabled(False); self.btn_stop.setEnabled(True); self._timer.start(100)
    def stop(self):
        self._running = False; self.btn_start.setEnabled(True); self.btn_stop.setEnabled(False); self._timer.stop()
    def _tick(self):
        ...
        for tag, sd in self._sim_data.items():
            ...
            if mode == "手动": continue
                ...
                freq = 1.0 / max(0.1, sd["period"]); amplitude = (sd["max"] - sd["min"]) / 2.0; center = (sd["max"] + sd["min"]) / 2.0
        ...
        for row in range(self.table.rowCount()):
            ...
            if tag in self._sim_data and self._sim_data[tag]["mode"] != "手动":
                val_str = f"{self._sim_data[tag]['value']:.2f}"; vi = self.table.item(row, 1)
    ...
    def _apply_value(self, tag, value):
        ...
        try:
            if isinstance(w, (QLabel, QLineEdit, QGroupBox)): w.setText(f"{value:.1f}")
            elif isinstance(w, (QProgressBar, QSlider)): w.setValue(int(value))
            elif isinstance(w, (QSpinBox, QDoubleSpinBox)): w.setValue(value)
            elif isinstance(w, QLCDNumber): w.display(value)
            elif isinstance(w, (QCheckBox, QRadioButton)): w.setChecked(value > (sd["max"] + sd["min"]) / 2.0)
            elif hasattr(w, 'addValue') and callable(getattr(w, 'addValue', None)): w.addValue(0, value)
        except Exception: pass
...
class UndoHistoryPanel(QWidget):
    def __init__(self, canvas, parent=None):
        super().__init__(parent); self.canvas = canvas; self.setMinimumWidth(180); self.setMaximumWidth(400)
        layout = QVBoxLayout(self); layout.setContentsMargins(0, 0, 0, 0); layout.setSpacing(4)
        ...
        btn_row = QHBoxLayout(); btn_row.setSpacing(4)
        self.btn_undo_step = QPushButton("↩ 回退一步"); self.btn_redo_step = QPushButton("↪ 前进一步"); self.btn_clear_history = QPushButton("清空历史")
        for btn in [self.btn_undo_step, self.btn_redo_step, self.btn_clear_history]:
            btn.setStyleSheet("QPushButton{padding:3px 8px; font-size:11px;}"); btn_row.addWidget(btn)
        btn_row.addStretch(); layout.addLayout(btn_row)
        self.btn_undo_step.clicked.connect(self.canvas.undo); self.btn_redo_step.clicked.connect(self.canvas.redo)
        self.btn_clear_history.clicked.connect(self._clear_history); self.list_widget.itemClicked.connect(self._on_item_clicked)
    def refresh(self):
        self.list_widget.clear()
        for cmd in self.canvas.history.undo_stack:
            desc = getattr(cmd, 'description', '操作用') or '操作'; item = QListWidgetItem(desc); item.setData(Qt.UserRole, id(cmd)); self.list_widget.addItem(item)
        if self.list_widget.count() > 0: self.list_widget.setCurrentRow(self.list_widget.count() - 1)
        self.btn_undo_step.setEnabled(self.canvas.history.can_undo()); self.btn_redo_step.setEnabled(self.canvas.history.can_redo())
    def _on_item_clicked(self, item):
        target_id = item.data(Qt.UserRole); target_idx = None
        for i, cmd in enumerate(self.canvas.history.undo_stack):
            if id(cmd) == target_id: target_idx = i; break
        if target_idx is None: return
        current_top = len(self.canvas.history.undo_stack) - 1; steps = current_top - target_idx
        for _ in range(steps): self.canvas.undo()
        self.canvas.widget_modified.emit()
    def _clear_history(self):
        self.canvas.history.undo_stack.clear(); self.canvas.history.redo_stack.clear(); self.canvas.widget_modified.emit()
...
class DeleteWidgetCmd(Command):
    def __init__(self, canvas, widget):
        self.canvas, self.widget = canvas, widget; self.display_name = widget.property("_display_name"); self.geo = widget.geometry()
        ...
        self.tag = widget.property("_tag") or ""; self.description = f"删除 {self.obj_name}"
    ...
    def undo(self):
        ...
        if w:
            ...
            w.setProperty("_locked", self.locked); w.setProperty("_designer_hidden", self.hidden); w.setProperty("_tag", self.tag)
...
class PropertyChangeCmd(Command):
    def __init__(self, canvas, widget, prop, old_val, new_val):
        self.canvas, self.widget, self.prop = canvas, widget, prop; self.old_val, self.new_val = old_val, new_val
...
class BatchPropertyChangeCmd(Command):
    def __init__(self, canvas, widgets, prop, old_vals, new_val):
        self.canvas, self.widgets, self.prop = canvas, widgets, prop; self.old_vals, self.new_val = old_vals, new_val
...
class ReorderWidgetCmd(Command):
    ...
    def execute(self):
        ly = self.container._content_layout; ly.removeWidget(self.widget); ly.insertWidget(self.new_index, self.widget); self.widget.show(); ly.update()
    def undo(self):
        ly = self.container._content_layout; ly.removeWidget(self.widget); ly.insertWidget(self.old_index, self.widget); self.widget.show(); ly.update()

class ExtractWidgetCmd(Command):
    def __init__(self, canvas, widget, container, index_in_layout):
        self.canvas, self.widget, self.container = canvas, widget, container; self.index_in_layout = index_in_layout; self.extract_pos = None
    ...
    def execute(self):
        ...
        self.widget.setParent(self.canvas); self.widget.move(self.extract_pos); self.widget.resize(self.widget.sizeHint()); self.widget.show()
        self.canvas._install_filter_recursive(self.widget); self.canvas.widget_modified.emit()
    def undo(self):
        self.widget.setParent(self.container); self.container._content_layout.insertWidget(self.index_in_layout, self.widget)
...
class _ReparentIntoContainerCmd(Command):
    def __init__(self, canvas, widget, container, canvas_index, layout_index):
        self.canvas = canvas; self.widget = widget; self.container = container; self.canvas_index = canvas_index; self.layout_index = layout_index
        self.description = f"移入容器 {widget.objectName()}"
    def execute(self):
        if self.widget in self.canvas._canvas_widgets: self.canvas._canvas_widgets.remove(self.widget)
        self.widget.setParent(self.container); ly = self.container._content_layout
        ly.insertWidget(min(self.layout_index, ly.count()), self.widget); self.widget.show(); ly.update()
    def undo(self):
        ly = self.container._content_layout; ly.removeWidget(self.widget); self.widget.setParent(self.canvas)
        self.canvas._canvas_widgets.insert(self.canvas_index, self.widget); self.widget.show(); self.canvas._install_filter_recursive(self.widget)

class _ReparentBetweenContainersCmd(Command):
    def __init__(self, canvas, widget, old_container, new_container, old_idx, new_idx):
        self.canvas = canvas; self.widget = widget; self.old_container = old_container; self.new_container = new_container; self.old_idx = old_idx; self.new_idx = new_idx
        self.description = f"容器间移动 {widget.objectName()}"
    def execute(self):
        old_ly = self.old_container._content_layout; old_ly.removeWidget(self.widget); self.widget.setParent(self.new_container)
        new_ly = self.new_container._content_layout; new_ly.insertWidget(min(self.new_idx, new_ly.count()), self.widget); self.widget.show(); new_ly.update()
    def undo(self):
        new_ly = self.new_container._content_layout; new_ly.removeWidget(self.widget); self.widget.setParent(self.old_container)
        old_ly = self.old_container._content_layout; old_ly.insertWidget(self.old_idx, self.widget); self.widget.show(); old_ly.update()
...
class CodeGenerator:
    ...
    def generate(canvas, as_ui_class=False, qss_filename="style.qss") -> str:
        ...
        canvas._save_current_page(); multi_page = canvas.page_count() > 1
        ...
        for imp in custom_imports: lines.insert(0, imp)
        if custom_imports: lines.insert(0, "")
        tag_bindings = CodeGenerator._collect_tag_bindings(canvas)
        if tag_bindings: lines.extend(CodeGenerator._generate_data_binder(tag_bindings)); lines.append("")
        ...
        obj_name_map = {}
        if multi_page: CodeGenerator._emit_multipage(canvas, lines, tag_bindings, obj_name_map)
        else: lines += ["        self.central_widget = QWidget()","        self.setCentralWidget(self.central_widget)",""]
        used = {}
        for w in canvas._canvas_widgets:
            if w.property("_designer_hidden"): continue
            CodeGenerator._emit(w, lines, used, "self.central_widget", "        ", obj_name_map)
        if tag_bindings:
            lines.append(""); lines.append("        # 初始化数据绑定管理器"); lines.append("        self.data_binder = DataBinder(self)")
            lines.append("        # TODO: 在此处启动您的通信线程，调用 self.data_binder.update_tag(tag, value) 刷新UI")
        if canvas._signal_connections:
            lines.append(""); lines.append("        # 信号/槽连接")
            for conn in canvas._signal_connections:
                sig = conn["signal"].split("(")[0]; src_var = obj_name_map.get(conn["source"], conn["source"])
            ...
            for conn in canvas._signal_connections:
                ...
                if slot not in generated_slots:
                    generated_slots.add(slot); callback_code = (canvas._callback_code or {}).get(slot, "").strip()
                    if callback_code:
                        if obj_name_map:
                        ...
                        lines.append(f"    def {slot}(self):"); lines.append(f"        # TODO: 实现 {slot}"); lines.append("        pass"); lines.append("")
        anchor_widgets = CodeGenerator._collect_anchor_widgets(canvas)
        if anchor_widgets: lines.append(""); lines.extend(CodeGenerator._generate_resize_event(anchor_widgets, used, canvas))
    ...
    def _emit_multipage(canvas, lines, tag_bindings, obj_name_map=None):
        pages = canvas._pages; page_names = [p["name"] for p in pages]; btn_width = 120
        lines += ["        # ── 多页面布局 ──","        self.central_widget = QWidget()","        self.setCentralWidget(self.central_widget)","        main_layout = QHBoxLayout(self.central_widget)","        main_layout.setContentsMargins(0, 0, 0, 0)","        main_layout.setSpacing(0)","","        # 侧边导航栏","        self.nav_widget = QWidget()",f"        self.nav_widget.setFixedWidth({btn_width + 20})","        self.nav_widget.setStyleSheet('QWidget{background:#2c3e50;}')","        nav_layout = QVBoxLayout(self.nav_widget)","        nav_layout.setContentsMargins(4, 8, 4, 8)","        nav_layout.setSpacing(4)","","        self.nav_buttons = []",f"        self.stack = QStackedWidget()",""]
        for i, pname in enumerate(page_names):
            safe_name = _sanitize(pname) or f"page{i}"
            lines += [f"        btn_{safe_name} = QPushButton('{_esc(pname)}')",f"        btn_{safe_name}.setFixedHeight(36)","        btn_{safe_name}.setStyleSheet('QPushButton{{color:#ecf0f1;background:transparent;border:none;text-align:left;padding:8px 12px;font-size:12px;border-radius:4px;}}QPushButton:hover{{background:#34495e;}}QPushButton:checked{{background:#4A90D9;font-weight:bold;}}')",f"        btn_{safe_name}.setCheckable(True)",f"        btn_{safe_name}.clicked.connect(lambda checked, idx={i}: self.stack.setCurrentIndex(idx))",f"        nav_layout.addWidget(btn_{safe_name})",f"        self.nav_buttons.append(btn_{safe_name})",f"        # ── 页面: {pname} ──",f"        page_{safe_name} = QWidget()",f"        self.stack.addWidget(page_{safe_name})",""]
        lines += ["        nav_layout.addStretch()","        main_layout.addWidget(self.nav_widget)","        main_layout.addWidget(self.stack)","        self.nav_buttons[0].setChecked(True) if self.nav_buttons else None",""]
        for i, p in enumerate(pages):
            safe_name = _sanitize(p["name"]) or f"page{i}"; used = {}
            for w in p["widgets"]:
                if w.property("_designer_hidden"): continue
                CodeGenerator._emit(w, lines, used, f"page_{safe_name}", "        ", obj_name_map)
        if tag_bindings: lines.append(""); lines.append("        # 初始化数据绑定管理器"); lines.append("        self.data_binder = DataBinder(self)"); lines.append("        # TODO: 启动通信线程，调用 self.data_binder.update_tag(tag, value) 刷新UI")

    @staticmethod
    def _collect_custom_imports(canvas):
        custom_cls_to_file = {}
        for display_name, cls, kwargs, filepath in CUSTOM_WIDGETS: custom_cls_to_file[cls] = filepath
        cls_to_file = {}
        def scan(widgets):
            for w in widgets:
                if w.property("_designer_hidden"): continue
                cls = type(w); mod = cls.__module__
                if cls in custom_cls_to_file: cls_to_file[cls] = custom_cls_to_file[cls]
                elif w.property("_custom_source"):
                    src = w.property("_custom_source")
                    if os.path.isfile(src): cls_to_file[cls] = src
                    ...
                    for display_name, ccls, kwargs, fp in CUSTOM_WIDGETS:
                        if ccls.__name__ == cls.__name__: cls_to_file[cls] = fp; break
                if hasattr(w, "_content_layout"):
                    children = [w._content_layout.itemAt(i).widget() for i in range(w._content_layout.count()) if w._content_layout.itemAt(i) and w._content_layout.itemAt(i).widget()]
                    scan(children)
        scan(canvas._canvas_widgets)
        for p in canvas._pages:
            if p["widgets"] is not canvas._canvas_widgets: scan(p["widgets"])
        file_to_classes = {}
        for cls, fp in cls_to_file.items():
            mod_name = os.path.splitext(os.path.basename(fp))[0]; file_to_classes.setdefault(mod_name, []).append(cls.__name__)
        imports = []
        for mod_name, class_names in file_to_classes.items(): imports.append(f"from {mod_name} import {', '.join(sorted(class_names))}")
        return imports
    ...
    def _collect_tag_bindings(canvas):
        ...
        def scan(widgets):
            for w in widgets:
                ...
                if hasattr(w, "_content_layout"):
                    ly = w._content_layout; children = [ly.itemAt(i).widget() for i in range(ly.count()) if ly.itemAt(i) and ly.itemAt(i).widget()]
                    scan(children)
        scan(canvas._canvas_widgets); return bindings

    @staticmethod
    def _generate_data_binder(bindings):
        lines = ["class DataBinder:",'    """',"    数据绑定管理器 - 由设计器自动生成","    外部通信线程通过 update_tag(tag, value) 统一刷新UI",'    """',"    def __init__(self, ui):","        self.ui = ui","        self._data = {}","","    def update_tag(self, tag: str, value):","        self._data[tag] = value"]
    ...
    def _var(widget, used):
        display_name = widget.property("_display_name") or ""; prefix = NAME_TO_PREFIX.get(display_name, _sanitize(type(widget).__name__))
    ...
    def _emit(w, lines, used, parent_ref, indent, obj_name_map=None):
        ...
        if obj_name_map is not None:
            ...
            if obj_name: obj_name_map[obj_name] = var
        ...
        if isinstance(w, (QPushButton, QCheckBox, QRadioButton, QCommandLinkButton)):
            ...
            if items:
                escaped_items = ", ".join([f'"{_esc(i)}"' for i in items]); post_init.append(f"self.{var}.addItems([{escaped_items}])")
        ...
        if hasattr(w, '_demo_timer') and hasattr(w, 'start'): pass
...
class WidgetToolbox(QWidget):
    def __init__(self, parent=None):
        ...
        self.search = QLineEdit(); self.search.setPlaceholderText("🔍 搜索控件..."); self.search.setClearButtonEnabled(True)
        self.search.setStyleSheet("QLineEdit{padding:4px 8px;border:1px solid #ccc;border-radius:4px;margin:2px 4px;font-size:12px;}")
        self.search.textChanged.connect(self._filter); layout.addWidget(self.search)
        self.list_widget = QListWidget(); self.list_widget.setDragEnabled(True); self.list_widget.setSelectionMode(QAbstractItemView.NoSelection)
        ...
        self.list_widget.setMaximumWidth(200); self.list_widget.setMinimumWidth(150); layout.addWidget(self.list_widget)
        self._all_items = []; self.list_widget.viewport().installEventFilter(self); self.populate()
    ...
    def eventFilter(self, obj, event):
        if obj is self.list_widget.viewport() and event.type() == QEvent.MouseMove:
            ...
            if item:
                ...
                if wt:
                    drag = QDrag(self); mime = QMimeData(); mime.setData(MIME_TYPE, wt.encode())
    ...
    def populate(self):
        ...
        for cat, items in get_all_categories():
            ...
            for name, _, _ in items: self._all_items.append((f"  {name}", False, cat, name))
        self._rebuild_list()
    def _rebuild_list(self, filter_text=""):
        self.list_widget.clear(); ft = filter_text.lower().strip(); visible_cat = None
        for text, is_header, cat, name in self._all_items:
            if is_header: visible_cat = None; continue
            if ft:
                if ft in text.lower() or ft in (name or "").lower() or ft in cat.lower():
                    if visible_cat != cat:
                        header_text = f"── {cat} ──"; h = QListWidgetItem(header_text); h.setFlags(Qt.NoItemFlags)
                        h.setForeground(QColor("#888")); f = QFont(); f.setBold(True); f.setPointSize(10); h.setFont(f)
                        self.list_widget.addItem(h); visible_cat = cat
                ...
                if visible_cat != cat:
                    ...
                    self.list_widget.addItem(h); visible_cat = cat
                item = QListWidgetItem(text); item.setData(Qt.UserRole, name); self.list_widget.addItem(item)
    def _filter(self, text): self._rebuild_list(text)
...
class DesignerCanvas(QWidget):
    selection_changed = Signal(object); widget_modified = Signal()
    def __init__(self, parent=None):
        ...
        self._signal_connections = []; self._callback_code = {}
        self._grid_enabled = True; self._zoom_factor = 1.0; self._preview_mode = False; self._fixed_canvas = False
        self._preview_btn = None; self.design_width = DESIGN_WIDTH; self.design_height = DESIGN_HEIGHT
        self._pages = [{"name": "页面1", "widgets": []}]; self._current_page = 0
        self._placeholder = _Placeholder("将左侧控件拖拽到此处\n拖拽选中 | Ctrl+点击多选 | G键切换网格 | 右键信号/槽", self)
        self._placeholder.setAlignment(Qt.AlignCenter); self._placeholder.setStyleSheet("color:#bbb; font-size:16px; background:transparent; border:none;")
        ...
        for pos in HANDLE_CURSORS:
            ...
            h.setVisible(False); h._is_handle = True; h._handle_pos = pos; h.installEventFilter(self); self._handles[pos] = h
    ...
    def _toggle_grid(self):
        self._grid_enabled = not self._grid_enabled; self.update()
        win = self.window()
        if hasattr(win, "grid_action"):
            win.grid_action.setChecked(self._grid_enabled); win.grid_action.setText("📐 网格:ON" if self._grid_enabled else "📐 网格:OFF")
        if hasattr(win, "statusBar"):
            state = "开启" if self._grid_enabled else "关闭"; win.statusBar().showMessage(f"网格已{state} (G键切换)")

    def enter_preview_mode(self):
        self._preview_mode = True; self._deselect()
        for h in self._handles.values(): h.setVisible(False)
        for w in self._canvas_widgets:
            w.setAttribute(Qt.WA_TransparentForMouseEvents, False)
            if hasattr(w, 'start') and hasattr(w, '_demo_timer'): w.start()
        self.setCursor(Qt.ArrowCursor)
        if self._preview_btn is None:
            self._preview_btn = QPushButton("✕ 退出预览 (ESC)", self)
            self._preview_btn.setStyleSheet("QPushButton{background:#E74C3C;color:#fff;border:none;border-radius:4px;padding:6px 14px;font-size:12px;font-weight:bold;}QPushButton:hover{background:#C0392B;}")
            self._preview_btn.clicked.connect(self.exit_preview_mode); self._preview_btn.setVisible(True); self._preview_btn.raise_(); self._preview_btn.move(self.width() - 140, 8)
        self._preview_shortcut = QShortcut(QKeySequence(Qt.Key_Escape), self); self._preview_shortcut.activated.connect(self.exit_preview_mode)
        self.update()
        win = self.window()
        if hasattr(win, "statusBar"): win.statusBar().showMessage("🔍 预览模式 — 控件可交互 | 按 ESC 退出")

    def exit_preview_mode(self):
        self._preview_mode = False
        if hasattr(self, '_preview_shortcut') and self._preview_shortcut: self._preview_shortcut.deleteLater(); self._preview_shortcut = None
        if self._preview_btn: self._preview_btn.setVisible(False)
        self.setCursor(Qt.ArrowCursor)
        for w in self._canvas_widgets:
            self._install_filter_recursive(w)
            if hasattr(w, 'stop') and hasattr(w, '_demo_timer'): w.stop()
        self.update()
        win = self.window()
        if hasattr(win, "statusBar"): win.statusBar().showMessage("✅ 已返回编辑模式 | G键切换网格")
        if hasattr(win, "btn_preview"): win.btn_preview.setChecked(False); win.btn_preview.setText("🔍 预览")
    ...
    def add_page(self, name=None):
        self._save_current_page(); name = name or f"页面{len(self._pages)+1}"
        self._pages.append({"name": name, "widgets": []}); self._current_page = len(self._pages) - 1
        self._canvas_widgets = self._pages[self._current_page]["widgets"]; self._deselect(); self.widget_modified.emit()
        return self._current_page
    def remove_page(self, idx):
        if len(self._pages) <= 1: return False
        self._save_current_page()
        for w in self._pages[idx]["widgets"]: w.hide(); w.deleteLater()
        del self._pages[idx]
        if self._current_page >= len(self._pages): self._current_page = len(self._pages) - 1
        self._canvas_widgets = self._pages[self._current_page]["widgets"]
        for w in self._canvas_widgets:
            if not w.property("_designer_hidden"): w.show()
        self._deselect(); self.widget_modified.emit(); return True
    def switch_page(self, idx):
        if idx == self._current_page or idx < 0 or idx >= len(self._pages): return
        self._save_current_page()
        for w in self._canvas_widgets: w.hide()
        self._current_page = idx; self._canvas_widgets = self._pages[idx]["widgets"]
        for w in self._canvas_widgets:
            if not w.property("_designer_hidden"): w.show()
        self._deselect(); self.widget_modified.emit()
    def rename_page(self, idx, name):
        if 0 <= idx < len(self._pages): self._pages[idx]["name"] = name; self.widget_modified.emit()
    def _save_current_page(self):
        if 0 <= self._current_page < len(self._pages): self._pages[self._current_page]["widgets"] = self._canvas_widgets
    ...
    def set_canvas_size(self, w, h):
        self.design_width = int(w); self.design_height = int(h); self.setMinimumSize(self.design_width, self.design_height)
        self.resize(self.design_width, self.design_height)
        if self._placeholder: self._placeholder.move((self.width()-self._placeholder.width())//2, (self.height()-self._placeholder.height())//2)
        self.update()
    ...
    def wheelEvent(self, event):
        ...
        if event.modifiers() & Qt.ControlModifier:
            ...
            self._zoom_factor = round(self._zoom_factor, 1); self._update_handles(); self.update()
            win = self.window()
            if hasattr(win, "statusBar"): win.statusBar().showMessage(f"🔍 缩放: {int(self._zoom_factor*100)}%  |  Ctrl+滚轮调节 | G切换网格")
            event.accept()
    ...
    def align_widgets(self, align_type):
        ...
        self.history.push(BatchAlignCmd(self, widgets, old_geos, new_geos)); self.widget_modified.emit(); self.update()
        self._update_status_bar(f"✅ 已对齐: {align_type} ({len(widgets)}个控件)")

    def insert_template(self, template_name):
        all_tmpl = {**INDUSTRIAL_TEMPLATES, **_load_user_templates()}; tmpl = all_tmpl.get(template_name)
        if not tmpl: return
        base_pos = QPoint(self.width() // 2 - 140, self.height() // 2 - 80)
        self.history.push(InsertTemplateCmd(self, tmpl["widgets"], base_pos)); self._update_status_bar(f"🏭 已插入模板: {template_name}")
            ...
            for w in ws:
                ...
                if hasattr(w, "_content_layout"):
                    for i in range(w._content_layout.count()):
                        ...
                        if item and item.widget(): all_widgets.append(item.widget())
        widgets_to_save = self._multi_selection if len(self._multi_selection) > 1 else [widget]; collect(widgets_to_save)
        def canvas_pos(w):
            p = w.pos(); parent = w.parent()
            while parent and parent is not self: p += parent.pos(); parent = parent.parent()
            return p
        min_x = min(canvas_pos(w).x() for w in all_widgets); min_y = min(canvas_pos(w).y() for w in all_widgets)
        tmpl_widgets = []
        for w in all_widgets:
            info = self._collect_widget_info(w); abs_pos = canvas_pos(w)
            info["x"] = abs_pos.x() - min_x; info["y"] = abs_pos.y() - min_y; tmpl_widgets.append(info)
        USER_TEMPLATES[name] = {"description": f"用户自定义: {name}", "widgets": tmpl_widgets}; _save_user_templates()
        self._update_status_bar(f"✅ 已保存模板: {name}（{len(tmpl_widgets)}个控件）")

    def _copy_selected(self):
        w = self._selected
        if not w: return
        self._clipboard_data = {"display_name": w.property("_display_name"), "role": w.property("role") or "default", "objectName": w.objectName(), "styleSheet": w.styleSheet(), "w": w.width(), "h": w.height(), "hSizePolicy": _policy_to_str(w.sizePolicy().horizontalPolicy()), "vSizePolicy": _policy_to_str(w.sizePolicy().verticalPolicy()), "minWidth": w.minimumWidth(), "minHeight": w.minimumHeight(), "maxWidth": w.maximumWidth(), "maxHeight": w.maximumHeight(), "tag": w.property("_tag") or "", "anchor_left": bool(w.property("_anchor_left")), "anchor_right": bool(w.property("_anchor_right")), "anchor_top": bool(w.property("_anchor_top")), "anchor_bottom": bool(w.property("_anchor_bottom"))}
    ...
    def _copy_style(self):
        w = self._selected
        if not w: return
        self._style_clipboard = {"role": w.property("role") or "default", "styleSheet": w.styleSheet()}
        if hasattr(w, 'font') and callable(w.font):
            f = w.font(); self._style_clipboard["fontFamily"] = f.family(); self._style_clipboard["fontSize"] = f.pointSize()
            self._style_clipboard["fontBold"] = f.bold(); self._style_clipboard["fontItalic"] = f.italic()
    ...
    def _paste_style(self):
        ...
        for w in targets:
            ...
            if "fontFamily" in sc and hasattr(w, 'setFont'):
                f = QFont(sc["fontFamily"], sc.get("fontSize", 9)); f.setBold(sc.get("fontBold", False)); f.setItalic(sc.get("fontItalic", False)); w.setFont(f)
        self.widget_modified.emit(); self.update(); self._update_status_bar(f"🎨 已粘贴样式到 {len(targets)} 个控件")

    def _paste_widget(self):
        if not self._clipboard_data: return
        data = self._clipboard_data; paste_pos = QPoint(self._selected.x()+10, self._selected.y()+10) if self._selected else QPoint(self.width()//2-60, self.height()//2-18)
        ...
        sp = w.sizePolicy(); sp.setHorizontalPolicy(SIZE_POLICY_MAP.get(data.get("hSizePolicy","Preferred"), QSizePolicy.Preferred))
        sp.setVerticalPolicy(SIZE_POLICY_MAP.get(data.get("vSizePolicy","Preferred"), QSizePolicy.Preferred)); w.setSizePolicy(sp)
        ...
        self.history.undo_stack.append(AddWidgetCmd.__new__(AddWidgetCmd)); self.history.undo_stack[-1].canvas = self
        self.history.undo_stack[-1].display_name = dn; self.history.undo_stack[-1].pos = paste_pos; self.history.undo_stack[-1].widget = w
        self.history.redo_stack.clear(); self.widget_modified.emit(); self._update_status_bar(f"📋 已粘贴: {w.objectName()}")
    ...
    def _arrow_move(self, dx, dy):
        ...
        old_pos = w.pos(); step = GRID_SIZE if self._grid_enabled else ARROW_STEP
        ...
        if self._grid_enabled: new_x = self._snap_to_grid(new_x); new_y = self._snap_to_grid(new_y)
        new_pos = QPoint(new_x, new_y)
        if new_pos != old_pos:
            self.history.push(MoveWidgetCmd(self, w, old_pos, new_pos)); self.widget_modified.emit(); self._update_handles(); self.update()
            self._update_status_bar(f"微调: {w.objectName()} → ({new_x}, {new_y})")

    def _toggle_preview_shortcut(self):
        win = self.window()
        if hasattr(win, 'btn_preview'): win.btn_preview.toggle()

    def _duplicate_selected(self):
        if not self._selected: return
        self._copy_selected()
        if self._clipboard_data:
            paste_pos = QPoint(self._selected.x()+10, self._selected.y()+10); dn = self._clipboard_data.get("display_name")
            if not dn: return
            entry_map = get_display_to_entry()
            if dn not in entry_map: return
            w = self._create_widget_internal(dn, paste_pos)
    ...
    def _calc_snap_guides(self, moving_rect, exclude_widget=None):
        ...
        if not in_container and len(self._canvas_widgets) >= 3:
            visible_widgets = sorted([w for w in self._canvas_widgets if w.isVisible() and w is not exclude_widget], key=lambda w: w.x())
            
            # --- 水平等间距检测 ---
            current_left = moving_rect.left(); current_right = moving_rect.right()
            ...
            if left_neighbor and right_neighbor:
                ...
                if abs(gap_left - gap_right) < SNAP_THRESHOLD * 2:
                    ...
                    if abs(current_left - ideal_x) < min_dx:
                        min_dx = abs(current_left - ideal_x); snap_x = ideal_x
                        guides.append(('eq_h', ideal_x)) 

            # --- 垂直等间距检测 ---
            visible_widgets_v = sorted([w for w in self._canvas_widgets if w.isVisible() and w is not exclude_widget], key=lambda w: w.y())
            current_top = moving_rect.top(); current_bottom = moving_rect.bottom()
            ...
            if top_neighbor and bottom_neighbor:
                ...
                if abs(gap_top - gap_bottom) < SNAP_THRESHOLD * 2:
                    ...
                    if abs(current_top - ideal_y) < min_dy:
                        min_dy = abs(current_top - ideal_y); snap_y = ideal_y
                        guides.append(('eq_v', ideal_y))

        # 3. 相对父容器居中吸附
        if in_container:
            cr = parent.geometry(); center_x = cr.left() + cr.width() // 2; center_y = cr.top() + cr.height() // 2
            if abs(moving_rect.center().x() - center_x) < SNAP_THRESHOLD: guides.append(('center_h', center_x))
            if abs(moving_rect.center().y() - center_y) < SNAP_THRESHOLD: guides.append(('center_v', center_y))
        else:
            boundary_x = [0, self.width()]; boundary_y = [0, self.height()]
            for ex in edges_x:
    ...
    def _create_widget_internal(self, display_name, drop_pos, center=True):
        ...
        if container: w.setParent(container); container._content_layout.addWidget(w)
            ...
            if self._grid_enabled: x = self._snap_to_grid(x); y = self._snap_to_grid(y)
    ...
    def _apply_property(self, widget, prop, value):
        try:
            if prop == "objectName": widget.setObjectName(value)
                ...
                vals[prop] = max(0, int(value)); widget._content_layout.setContentsMargins(vals["marginLeft"], vals["marginTop"], vals["marginRight"], vals["marginBottom"]); widget.update()
            elif prop == "spacing" and hasattr(widget, "_content_layout"):
                widget._content_layout.setSpacing(max(0, int(value))); widget.update()
    ...
    def _collect_widget_info(self, w):
        sp = w.sizePolicy(); value_attr = None
        if hasattr(w, "value") and callable(getattr(w, "value", None)):
            try:
                val = w.value()
                if isinstance(val, (int, float, bool, str)): value_attr = val
                elif isinstance(val, (list, tuple)): value_attr = [self._to_json_safe(v) for v in val]
                else: value_attr = str(val)
            except Exception: value_attr = None
        text_attr = None
        try:
            if hasattr(w, "text") and not isinstance(w, QGroupBox):
                t = w.text(); text_attr = t if isinstance(t, str) else str(t) if t is not None else None
        except Exception: text_attr = None
        title_attr = None
        try:
            if hasattr(w, "title") and isinstance(w, QGroupBox):
                t = w.title(); title_attr = t if isinstance(t, str) else str(t) if t is not None else None
        except Exception: title_attr = None
        placeholder_attr = None
        try:
            if isinstance(w, (QLineEdit, QWTextEdit)): placeholder_attr = w.placeholderText() or ""
        except Exception: placeholder_attr = ""
        info = {"display_name": self._to_json_safe(w.property("_display_name")), "x": w.x(), "y": w.y(), "w": w.width(), "h": w.height(), "objectName": self._to_json_safe(w.objectName()), "role": self._to_json_safe(w.property("role") or "default"), "styleSheet": self._to_json_safe(w.styleSheet()), "hSizePolicy": _policy_to_str(sp.horizontalPolicy()), "vSizePolicy": _policy_to_str(sp.verticalPolicy()), "minWidth": w.minimumWidth(), "minHeight": w.minimumHeight(), "maxWidth": w.maximumWidth(), "maxHeight": w.maximumHeight(), "locked": bool(w.property("_locked")), "hidden": bool(w.property("_designer_hidden")), "tag": self._to_json_safe(w.property("_tag") or ""), "anchor_left": bool(w.property("_anchor_left")), "anchor_right": bool(w.property("_anchor_right")), "anchor_top": bool(w.property("_anchor_top")), "anchor_bottom": bool(w.property("_anchor_bottom"))}
        ...
        if hasattr(w, "_content_layout"):
            children = []; ly = w._content_layout
            for i in range(ly.count()):
                item = ly.itemAt(i)
                if item and item.widget():
                    child_w = item.widget()
                    if not child_w.property("_designer_hidden"): children.append(self._collect_widget_info(child_w))
    ...
    def save_project(self, path):
        ...
        try:
            self._save_current_page(); pages_data = []
            for pi, p in enumerate(self._pages):
                pw_list = []
                for wi, w in enumerate(list(p["widgets"])):
                    try: info = self._collect_widget_info(w); pw_list.append(info)
                    except Exception as e: _tb.print_exc()
                pages_data.append({"name": p["name"], "widgets": pw_list})
            data = {"pages": pages_data, "signal_connections": self._signal_connections, "callback_code": self._callback_code, "design_width": self.design_width, "design_height": self.design_height}
            with open(path, "w", encoding="utf-8") as f: json.dump(data, f, ensure_ascii=False, indent=2)
        except Exception as e: _tb.print_exc(); raise
    ...
    def load_project(self, path):
        ...
        for p in self._pages:
            for w in p.get("widgets", []): w.hide(); w.deleteLater()
        self._pages = [{"name": "页面1", "widgets": []}]; self._current_page = 0; self._canvas_widgets = self._pages[0]["widgets"]
        if isinstance(raw, list): self._signal_connections = []; self._load_page_widgets(raw, 0)
        else:
            self._signal_connections = raw.get("signal_connections", []); self._callback_code = raw.get("callback_code", {})
            if "design_width" in raw: self.set_canvas_size(raw["design_width"], raw["design_height"])
            pages_data = raw.get("pages", [])
            if pages_data:
                self._pages = []
                for i, pd in enumerate(pages_data):
                    name = pd.get("name", f"页面{i+1}"); self._pages.append({"name": name, "widgets": []})
                    self._current_page = i; self._canvas_widgets = self._pages[i]["widgets"]; self._load_page_widgets(pd.get("widgets", []), i)
                self._current_page = 0; self._canvas_widgets = self._pages[0]["widgets"]
                for w in self._canvas_widgets:
                    if not w.property("_designer_hidden"): w.show()
            else: self._load_page_widgets(raw.get("widgets", []), 0)
        self._deselect(); self.widget_modified.emit()
    ...
    def _load_page_widgets(self, widget_data, page_idx):
        for info in widget_data:
            ...
            pos = QPoint(info.get("x",0), info.get("y",0)); w = self._create_widget_internal(dn, pos)
            if not w: continue
            ...
            if "hSizePolicy" in info or "vSizePolicy" in info:
                sp = w.sizePolicy(); sp.setHorizontalPolicy(SIZE_POLICY_MAP.get(info.get("hSizePolicy","Preferred"), QSizePolicy.Preferred))
                sp.setVerticalPolicy(SIZE_POLICY_MAP.get(info.get("vSizePolicy","Preferred"), QSizePolicy.Preferred)); w.setSizePolicy(sp)
    ...
    def _select(self, w, additive=False):
        if additive and w in self._multi_selection:
            self._multi_selection.remove(w); self._selected = self._multi_selection[-1] if self._multi_selection else None
        elif additive: self._multi_selection.append(w); self._selected = w
        else:
    ...
    def paintEvent(self, e):
        super().paintEvent(e)
        if self._preview_mode: return
        p = QPainter(self); p.scale(self._zoom_factor, self._zoom_factor)
        if self._grid_enabled:
            w, h = self.width(), self.height(); end_x = int(w / self._zoom_factor) + GRID_SIZE; end_y = int(h / self._zoom_factor) + GRID_SIZE
            p.setPen(QPen(QColor("#e8e8e8"), 1, Qt.SolidLine))
            for x in range(0, end_x, GRID_SIZE): p.drawLine(x, 0, x, end_y)
            for y in range(0, end_y, GRID_SIZE): p.drawLine(0, y, end_x, y)
            p.setPen(QPen(QColor("#d0d0d0"), 1, Qt.SolidLine)); big = GRID_SIZE * 5
            for x in range(0, end_x, big): p.drawLine(x, 0, x, end_y)
            for y in range(0, end_y, big): p.drawLine(0, y, end_x, y)
        ...
        if self._snap_guides:
            for axis, pos in self._snap_guides:
                if axis.startswith('eq_'): p.setPen(QPen(QColor("#27AE60"), 2, Qt.DashLine))      # 等间距：绿色虚线
                elif axis.startswith('center_'): p.setPen(QPen(QColor("#4A90D9"), 1, Qt.DotLine)) # 容器居中：蓝色点线
                else: p.setPen(QPen(QColor("#E74C3C"), 1, Qt.SolidLine))                          # 普通对齐：红色实线
                
                if 'h' in axis or axis == 'x': p.drawLine(pos, 0, pos, self.height())
                if 'v' in axis or axis == 'y': p.drawLine(0, pos, self.width(), pos)
        ...
        if self._rubber_band_rect.isValid():
            p.setPen(QPen(QColor("#4A90D9"), 1, Qt.DashLine)); p.setBrush(QBrush(QColor(74, 144, 217, 30)))
            p.drawRect(self._rubber_band_rect)
        p.end()

    def eventFilter(self, obj, event):
        if self._preview_mode:
            if event.type() == QEvent.ShortcutOverride and event.key() == Qt.Key_Escape: return False
            return super().eventFilter(obj, event)
        et = event.type()
        if et == QEvent.Wheel and event.modifiers() & Qt.ControlModifier:
            ...
            self._zoom_factor = round(self._zoom_factor, 1); self._update_handles(); self.update()
            win = self.window()
            if hasattr(win, "statusBar"): win.statusBar().showMessage(f"🔍 缩放: {int(self._zoom_factor*100)}%  |  Ctrl+滚轮调节")
            return True
        if et == QEvent.ContextMenu:
            target = self._find_target_widget(obj)
            if target:
                if target not in self._multi_selection or len(self._multi_selection) <= 1: self._select(target)
                global_pos = event.globalPos(); QTimer.singleShot(0, lambda gp=global_pos: self._show_context_menu(gp)); return True
            return super().eventFilter(obj, event)
        ...
        if et==QEvent.MouseButtonPress and event.button()==Qt.LeftButton:
            ...
            if target:
                ...
                if target in self._multi_selection and len(self._multi_selection) > 1 and not ctrl_pressed: self._selected = target
                else: self._select(target, additive=ctrl_pressed)
                ...
                if not in_container and not target.property("_locked"):
                    ...
                    for mw in self._multi_selection:
                        p = mw.parent()
                        if not (p and hasattr(p, "_content_layout")) and not mw.property("_locked"): self._drag_start_geoms[id(mw)] = QRect(mw.geometry())
                return True
        ...
        if et==QEvent.MouseButtonRelease and self._move_active:
            ...
            if hasattr(self, '_drag_start_geoms') and len(self._drag_start_geoms) > 1:
                ...
                for mw in self._multi_selection:
                    ...
                    if new_g.topLeft() != sg.topLeft(): moved.append(mw); old_geos.append(sg); new_geos.append(new_g)
                if moved: self.history.push(BatchAlignCmd(self, moved, old_geos, new_geos)); self.widget_modified.emit()
            else:
    ...
    def mousePressEvent(self, e):
        ...
        if e.button() == Qt.LeftButton:
            self._rubber_band_active = True; self._rubber_band_start = e.position().toPoint(); self._rubber_band_rect = QRect()
        super().mousePressEvent(e)

    def mouseMoveEvent(self, e):
        if self._rubber_band_active and e.buttons() & Qt.LeftButton:
            p = e.position().toPoint()
            self._rubber_band_rect = QRect(min(self._rubber_band_start.x(), p.x()), min(self._rubber_band_start.y(), p.y()), abs(p.x() - self._rubber_band_start.x()), abs(p.y() - self._rubber_band_start.y()))
            self.update()
        super().mouseMoveEvent(e)

    def mouseReleaseEvent(self, e):
        if self._rubber_band_active and e.button() == Qt.LeftButton:
            self._rubber_band_active = False; sel = []
            for w in self._canvas_widgets:
                if w.isVisible() and not w.property("_designer_hidden"):
                    if self._rubber_band_rect.intersects(w.geometry()): sel.append(w)
            if sel:
                self._deselect(); self._multi_selection = sel; self._selected = sel[-1]
                self._update_handles(); self.selection_changed.emit(self._selected)
                self._update_status_bar(f"已选中 {len(sel)} 个控件，点击任意一个拖动")
            self._rubber_band_rect = QRect(); self.update()
        super().mouseReleaseEvent(e)

    def _show_context_menu(self, global_pos):
        w = self._selected
        if not w: return
        menu = QMenu(self); parent = w.parent(); in_container = parent and hasattr(parent, "_content_layout"); action_map = {}
        act_signals = menu.addAction("🔗 信号/槽编辑器..."); action_map[act_signals] = "signals"; menu.addSeparator()
        if in_container:
            ...
            act_down = menu.addAction("⬇️ 下移"); act_down.setEnabled(idx<count-1); action_map[act_down] = "down"; menu.addSeparator()
            act_extract = menu.addAction("📤 移出容器"); action_map[act_extract] = "extract"; menu.addSeparator()
        locked = bool(w.property("_locked")); act_lock = menu.addAction(f"{'🔓 解锁' if locked else '🔒 锁定'}"); action_map[act_lock] = "lock"
        hidden = bool(w.property("_designer_hidden")); act_hide = menu.addAction(f"{'👁️ 显示' if hidden else '👁️‍🗨️ 隐藏'}"); action_map[act_hide] = "hide"; menu.addSeparator()
        act_save_template = menu.addAction("📦 另存为模板"); action_map[act_save_template] = "save_template"; menu.addSeparator()
        ...
        act_paste_style = menu.addAction("🖌️ 粘贴样式"); act_paste_style.setEnabled(self._style_clipboard is not None); action_map[act_paste_style] = "paste_style"; menu.addSeparator()
        act_copy = menu.addAction("📋 复制 objectName"); action_map[act_copy] = "copy"; menu.addSeparator()
        callback_menu = menu.addMenu("✏️ 编辑回调函数")
        widget_name = w.objectName(); related_connections = [c for c in self._signal_connections if c["source"] == widget_name]
        if related_connections:
            for conn in related_connections:
                slot_name = conn["slot"]; has_code = slot_name in self._callback_code and self._callback_code[slot_name].strip()
                label = f"  {slot_name}" + (" ✅" if has_code else ""); act_cb = callback_menu.addAction(label); act_cb.setData(slot_name); action_map[act_cb] = ("edit_callback", slot_name)
        else:
            act_no_conn = callback_menu.addAction(" (暂无连接，请先添加信号/槽...)"); act_no_conn.setEnabled(False); action_map[act_no_conn] = "noop"
        ...
        if isinstance(at, tuple) and at[0] == "edit_callback":
            slot_name = at[1]; existing_code = self._callback_code.get(slot_name, "")
            dlg = CallbackEditorDialog(self, slot_name, existing_code, self.window())
            if dlg.exec() == QDialog.Accepted: self._callback_code[slot_name] = dlg.get_code(); self.widget_modified.emit()
        elif at == "signals": dlg = SignalSlotDialog(self, self.window()); dlg.exec()
    ...
    def clear_canvas(self):
        ...
        self._pages = [{"name": "页面1", "widgets": []}]; self._current_page = 0; self._canvas_widgets = self._pages[0]["widgets"]
        self._signal_connections.clear(); self._callback_code.clear(); self._placeholder.setVisible(True); self.widget_modified.emit()
...
class PropertyEditor(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.table.verticalHeader().setVisible(False); self.table.setEditTriggers(QAbstractItemView.DoubleClicked | QAbstractItemView.EditKeyPressed)
        ...
        self.table.cellChanged.connect(self._on_cell_changed); self.table.cellDoubleClicked.connect(self._on_cell_double_clicked)
...
# ══════════════════════════════════════════════════════════════
# 找到 MultiTrendWidget 类并替换为以下代码

class MultiTrendWidget(QWidget):
    """工业级多通道实时趋势曲线"""
    _display_name = "多通道趋势曲线" # 保持显示名称一致
    
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setAttribute(Qt.WA_OpaquePaintEvent, True)
        self.setAttribute(Qt.WA_NoSystemBackground, False) # 确保背景绘制
        self.setMinimumSize(200, 150)
    ...
    def addValue(self, channel_index, value):
        if 0 <= channel_index < len(self._channels):
            ...
            # 无论是否运行，只要有新数据就尝试更新，确保在编辑模式下也能看到变化（如果手动触发）
            # 但在预览模式下，主要由定时器驱动
            if self.isVisible():
                self.update()
    ...
    def start(self):
        ...
        if not self._running:
            ...
            # print(f"[Curve] Started: {self.objectName()}") # 调试用
    ...
    def stop(self):
        ...
        if self._running:
            self._running = False
            self._demo_timer.stop()
            # print(f"[Curve] Stopped: {self.objectName()}") # 调试用
    ...
    def paintEvent(self, event):
        ...
        # 【关键修复】保存并重置变换矩阵，防止受 DesignerCanvas 的 scale() 影响
        p.save()
        p.resetTransform() 
        ...
        # 绘制 X 轴时间标签
        time_step = self._x_range / self._grid_density_x
        for i in range(self._grid_density_x + 1):
            t_sec = i * time_step
            gx = ml + pw * i / self._grid_density_x
...
class PropertyEditor(QWidget):
    ...
    def _refresh(self):
        ...
        if cls_name == "MultiTrendWidget":
            props += [
                ("x_range", str(w._x_range), True),          # X轴显示时长(秒)
                ("y_min", str(w._y_min), True),              # Y轴最小值
                ("y_max", str(w._y_max), True),              # Y轴最大值
                ("auto_scale_y", "✓" if w._auto_scale_y else "", True), # 自动缩放Y轴
                ("grid_density_x", str(w._grid_density_x), True),     # X轴网格线数
                ("grid_density_y", str(w._grid_density_y), True),     # Y轴网格线数
                ("bg_color", w._bg_color.name(), True),      # 背景颜色
                ("grid_color", w._grid_color.name(), True),  # 网格颜色
                ("text_color", w._text_color.name(), True),  # 文字颜色
            ]
...
if __name__ == "__main__":
    main()
    def _refresh_batch(self):
        widgets = self._batch_widgets; candidate_props = [("role", lambda w: type(w).__name__ in WIDGET_ROLES), ("text", lambda w: hasattr(w, "text") and not isinstance(w, (QGroupBox, QDialogButtonBox))), ("title", lambda w: hasattr(w, "title") and isinstance(w, QGroupBox)), ("placeholderText", lambda w: isinstance(w, (QLineEdit, QWTextEdit))), ("tag", lambda w: isinstance(w, BINDABLE_WIDGETS) or hasattr(w, 'setValue')), ("width", lambda w: True), ("height", lambda w: True), ("minWidth", lambda w: True), ("minHeight", lambda w: True), ("maxWidth", lambda w: True), ("maxHeight", lambda w: True), ("styleSheet", lambda w: True)]
        props = []
        for prop_name, condition in candidate_props:
            applicable = [w for w in widgets if condition(w)]
            if not applicable: continue
            values = [self._get_widget_value(w, prop_name) for w in applicable]; values = [v for v in values if v is not None]
            if not values: continue
            all_same = all(v == values[0] for v in values); display_val = values[0] if all_same else "(混合)"; props.append((prop_name, display_val, True))
        if not props:
            self.table.setRowCount(1); ki = QTableWidgetItem("提示"); ki.setFlags(ki.flags() & ~Qt.ItemIsEditable); ki.setBackground(QColor("#f0f0f0")); self.table.setItem(0, 0, ki)
            vi = QTableWidgetItem("选中控件无公共可编辑属性"); vi.setFlags(vi.flags() & ~Qt.ItemIsEditable); vi.setForeground(QColor("#999")); self.table.setItem(0, 1, vi); return
        self._render_props(props, None)
    PROP_GROUPS = {"📐 位置与尺寸": ("x", "y", "width", "height", "anchor_left", "anchor_right", "anchor_top", "anchor_bottom"), "🎨 样式": ("role", "styleSheet"), "📝 内容与标识": ("objectName", "text", "title", "placeholderText", "value", "tag", "unit"), "📏 大小策略": ("minWidth", "minHeight", "maxWidth", "maxHeight", "hSizePolicy", "vSizePolicy"), "📦 容器边距": ("marginLeft", "marginTop", "marginRight", "marginBottom", "spacing")}
    def _get_group(self, prop_name):
        for group_name, props in self.PROP_GROUPS.items():
            if prop_name in props: return group_name
        return "其他"
    def _render_props(self, props, single_widget):
        row = 0; current_group = None
        for prop, value, _ in props:
            group = self._get_group(prop)
            if group != current_group:
                current_group = group; self.table.insertRow(row); ki = QTableWidgetItem(f"── {group} ──"); ki.setFlags(Qt.NoItemFlags)
                f = ki.font(); f.setBold(True); f.setPointSize(10); ki.setFont(f); ki.setBackground(QColor("#e0e0e0")); self.table.setItem(row, 0, ki)
                vi = QTableWidgetItem(""); vi.setFlags(Qt.NoItemFlags); vi.setBackground(QColor("#e0e0e0")); self.table.setItem(row, 1, vi); row += 1
            self.table.insertRow(row); ki = QTableWidgetItem(prop); ki.setFlags(ki.flags() & ~Qt.ItemIsEditable); ki.setBackground(QColor("#f5f5f5")); self.table.setItem(row, 0, ki)
            if prop == "objectName" and single_widget:
                vi = QTableWidgetItem(str(value) if value is not None else ""); is_auto = single_widget.property("_auto_objectName")
                if is_auto: f = vi.font(); f.setItalic(True); vi.setFont(f); vi.setForeground(QColor("#999")); vi.setToolTip("此为自动生成的临时名称，设置tag后将自动同步为语义化名称")
                self.table.setItem(row, 1, vi)
            elif prop in ("anchor_left", "anchor_right", "anchor_top", "anchor_bottom") and single_widget:
                cb = QCheckBox(); cb.setChecked(value == "✓"); cb.stateChanged.connect(lambda state, widget=single_widget, p=prop: self._on_anchor_changed(widget, p, state)); self.table.setCellWidget(row, 1, cb)
    ...
    def _apply_batch_property(self, prop, value):
        ...
        for w in self._batch_widgets:
            ...
            if ov is not None: old_vals.append(ov); applicable_widgets.append(w)
        if not applicable_widgets: return
        canvas.history.push(BatchPropertyChangeCmd(canvas, applicable_widgets, prop, old_vals, value)); canvas.widget_modified.emit(); self.property_changed.emit()
    def _on_anchor_changed(self, widget, prop, state):
        if self._busy or not widget: return
        checked = state == Qt.Checked; old_val = "✓" if widget.property(f"_{prop}") else ""; new_val = "✓" if checked else ""
        if old_val == new_val: return
    ...
    def _on_cell_double_clicked(self, row, col):
        if col != 1: return
        prop_item = self.table.item(row, 0)
        if not prop_item or prop_item.text() != "styleSheet": return
        if not self._widget: return
        dlg = QssBuilderDialog(self._widget, self.window())
        if dlg.exec() == QDialog.Accepted:
            new_qss = dlg.get_qss(); canvas = self._widget.parent()
            while canvas and not isinstance(canvas, DesignerCanvas): canvas = canvas.parent()
            if canvas:
                old_qss = self._widget.styleSheet(); canvas.history.push(PropertyChangeCmd(canvas, self._widget, "styleSheet", old_qss, new_qss))
                self._refresh(); self.property_changed.emit()
    def _on_cell_changed(self, row, col):
        if self._busy or col != 1: return
        prop = self.table.item(row, 0).text()
        ...
        try:
            if prop == "objectName": old_val = w.objectName(); w.setProperty("_auto_objectName", False)
                ...
                m = w._content_layout.contentsMargins(); vals = {"marginLeft": m.left(), "marginTop": m.top(), "marginRight": m.right(), "marginBottom": m.bottom()}; old_val = str(vals.get(prop, 0))
            elif prop == "spacing" and hasattr(w, "_content_layout"): old_val = str(w._content_layout.spacing())
            elif prop == "styleSheet": old_val = w.styleSheet()
        except: old_val = ""
        if str(old_val) == value: return
        canvas.history.push(PropertyChangeCmd(canvas, w, prop, old_val, value)); self._refresh(); self.property_changed.emit()
    def refresh(self): self._refresh()

# ── 主窗口 ───────────────────────────────────────────────────────
class DesignerMainWindow(QMainWindow):
    def __init__(self):
        super().__init__(); self.setWindowTitle("Mini Designer v20 — 智能吸附/内置曲线/热重载"); self._current_file = None; self._ui_class_mode = False; self._custom_widget_files = []
        self._setup_ui(); QShortcut(QKeySequence("Ctrl+S"), self).activated.connect(self._save_project)
        QShortcut(QKeySequence("Ctrl+N"), self).activated.connect(self._new_project); QShortcut(QKeySequence("Ctrl+O"), self).activated.connect(self._load_project)
        QApplication.instance().setStyle(QStyleFactory.create("Fusion")); QApplication.instance().setStyleSheet(ACTIVE_QSS)
        self._wire(); self._restore_state(); self._auto_load_custom_widgets(); self._init_git()

    def _setup_ui(self):
        central = QWidget(); self.setCentralWidget(central); root = QVBoxLayout(central); root.setContentsMargins(4, 4, 4, 4); root.setSpacing(0)
        self.align_toolbar = QToolBar("对齐工具"); self.align_toolbar.setMovable(False)
        for label, at in [("⬅️左","left"),("➡️右","right"),("⬆️顶","top"),("⬇️底","bottom"),("↔️水平居中","hcenter"),("↕️垂直居中","vcenter"),("═等宽","same_width"),("║等高","same_height"),("⋯水平等距","distribute_h"),("⋮垂直等距","distribute_v")]:
            action = self.align_toolbar.addAction(label); action.triggered.connect(lambda checked, a=at: self.canvas.align_widgets(a))
        self.align_toolbar.addSeparator(); self.grid_action = self.align_toolbar.addAction("📐 网格:ON"); self.grid_action.setCheckable(True); self.grid_action.setChecked(True); self.grid_action.triggered.connect(self._toggle_grid_action)
        self.addToolBar(Qt.TopToolBarArea, self.align_toolbar)
        top_split = QSplitter(Qt.Horizontal); self.toolbox = WidgetToolbox(); self.canvas = DesignerCanvas(); self.props = PropertyEditor()
        self.props.setMinimumWidth(220); self.props.setMaximumWidth(400)
        left_tabs = QTabWidget(); left_tabs.addTab(self.toolbox, "🧰 工具箱"); self.widget_tree = WidgetTreePanel(self.canvas); left_tabs.addTab(self.widget_tree, "📁 控件树")
        right_tabs = QTabWidget(); right_tabs.addTab(self.props, "📋 属性"); self.data_sim = DataSimulatorPanel(self.canvas); right_tabs.addTab(self.data_sim, "🔬 模拟器")
        self.history_panel = UndoHistoryPanel(self.canvas); right_tabs.addTab(self.history_panel, "📜 历史")
        try:
            from comm_debug import CommDebugPanel; self.comm_panel = CommDebugPanel(self.canvas); right_tabs.addTab(self.comm_panel, "🔌 通信")
        except: pass
        canvas_area = QWidget(); canvas_area_layout = QVBoxLayout(canvas_area); canvas_area_layout.setContentsMargins(0, 0, 0, 0); canvas_area_layout.setSpacing(0)
        page_bar = QWidget(); page_bar.setStyleSheet("background:#e8e8e8;border-bottom:1px solid #ccc;"); page_bar_layout = QHBoxLayout(page_bar); page_bar_layout.setContentsMargins(4, 2, 4, 2); page_bar_layout.setSpacing(2)
        self.page_tabs = QTabBar(); self.page_tabs.setTabsClosable(True); self.page_tabs.setMovable(True); self.page_tabs.setExpanding(False)
        self.page_tabs.setToolTip("双击标签可重命名 | 拖拽可排序 | 点击 ⨉ 删除")
        self.page_tabs.setStyleSheet("QTabBar::tab{min-width:80px;padding:6px 14px;font-size:12px;font-weight:bold;background:#c8c8c8;border:1px solid #aaa;border-bottom:none;border-radius:6px 6px 0 0;margin-right:3px;color:#333;}QTabBar::tab:selected{background:#fff;color:#4A90D9;border-bottom:2px solid #4A90D9;}QTabBar::tab:hover{background:#d8d8d8;}")
        self.page_tabs.addTab("页面1"); btn_add_page = QPushButton("＋ 新建页"); btn_add_page.setMinimumWidth(80)
        btn_add_page.setStyleSheet("QPushButton{background:#4A90D9;color:#fff;border:none;border-radius:4px;padding:3px 10px;font-size:12px;font-weight:bold;} QPushButton:hover{background:#357ABD;}")
        page_bar_layout.addWidget(self.page_tabs); page_bar_layout.addWidget(btn_add_page); page_bar_layout.addStretch(); canvas_area_layout.addWidget(page_bar)
        self.canvas_scroll = QScrollArea(); self.canvas_scroll.setWidgetResizable(False); self.canvas_scroll.setWidget(self.canvas)
        self.canvas_scroll.setStyleSheet("QScrollArea{background:#e0e0e0;border:none;}"); self.canvas.setMinimumSize(DESIGN_WIDTH, DESIGN_HEIGHT); canvas_area_layout.addWidget(self.canvas_scroll)
        self.page_tabs.currentChanged.connect(self._on_page_tab_changed); self.page_tabs.tabCloseRequested.connect(self._on_page_close)
        self.page_tabs.tabBarDoubleClicked.connect(self._on_page_rename); btn_add_page.clicked.connect(self._on_add_page)
        top_split.addWidget(left_tabs); top_split.addWidget(canvas_area); top_split.addWidget(right_tabs); top_split.setSizes([200, 700, 300])
        bottom = QWidget(); bl = QVBoxLayout(bottom); bl.setContentsMargins(0, 4, 0, 0); bl.setSpacing(4)
        BUTTON_STYLE = "QToolButton{padding:4px 8px;font-size:11px;}"
        self.toolbar_row1 = QToolBar("常用工具栏"); self.toolbar_row1.setMovable(False); self.toolbar_row1.setStyleSheet("QToolBar{spacing:2px;padding:1px 4px;border:none;}")
        self.btn_template = self._add_tb_btn2(self.toolbar_row1, "🏭 模板", BUTTON_STYLE)
        self.btn_undo = self._add_tb_btn2(self.toolbar_row1, "↩ 撤销", BUTTON_STYLE); self.btn_undo.setEnabled(False)
        self.btn_redo = self._add_tb_btn2(self.toolbar_row1, "↪ 重做", BUTTON_STYLE); self.btn_redo.setEnabled(False)
        self.btn_canvas_size = self._add_tb_btn2(self.toolbar_row1, "📐 800×600", BUTTON_STYLE); self.toolbar_row1.addSeparator()
        self.btn_preview = self._add_tb_btn2(self.toolbar_row1, "🔍 预览", "QToolButton{background:#27AE60;color:#fff;border:none;padding:4px 8px;border-radius:4px;font-weight:bold;font-size:11px;} QToolButton:hover{background:#219A52;} QToolButton:checked{background:#E74C3C;}"); self.btn_preview.setCheckable(True)
        bl.addWidget(self.toolbar_row1)
        self.toolbar_row2 = QToolBar("工具/文件"); self.toolbar_row2.setMovable(False); self.toolbar_row2.setStyleSheet("QToolBar{spacing:2px;padding:1px 4px;border:none;}")
        self.btn_dark = self._add_tb_btn2(self.toolbar_row2, "🌙 暗色", BUTTON_STYLE); self.btn_dark.setCheckable(True)
        self.btn_custom = self._add_tb_btn2(self.toolbar_row2, "🧩 自定义控件", BUTTON_STYLE); self.btn_theme = self._add_tb_btn2(self.toolbar_row2, "🎨 主题", BUTTON_STYLE)
        self.btn_ui_class = self._add_tb_btn2(self.toolbar_row2, "🪟 独立窗口", BUTTON_STYLE); self.btn_ui_class.setCheckable(True)
        self.btn_fixed = self._add_tb_btn2(self.toolbar_row2, "🔒 固定画布", BUTTON_STYLE); self.btn_fixed.setCheckable(True); self.toolbar_row2.addSeparator()
        self.btn_save = self._add_tb_btn2(self.toolbar_row2, "💾 保存", BUTTON_STYLE); self.btn_load = self._add_tb_btn2(self.toolbar_row2, "📂 打开", BUTTON_STYLE)
        self.btn_export_project = self._add_tb_btn2(self.toolbar_row2, "📦 导出", BUTTON_STYLE); self.btn_export_qss = self._add_tb_btn2(self.toolbar_row2, "导出 QSS", BUTTON_STYLE); self.toolbar_row2.addSeparator()
        self.btn_clear = self._add_tb_btn2(self.toolbar_row2, "🗑️ 清空", BUTTON_STYLE); bl.addWidget(self.toolbar_row2)
        code_header = QWidget(); code_header_l = QHBoxLayout(code_header); code_header_l.setContentsMargins(4, 0, 4, 2); code_header_l.setSpacing(6)
        code_label = QLabel("<b>生成代码</b>"); code_label.setStyleSheet("font-size: 12px; color: #555;"); code_header_l.addWidget(code_label); code_header_l.addStretch()
        self.btn_copy = QPushButton("📋 复制代码"); self.btn_copy.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Fixed); code_header_l.addWidget(self.btn_copy); bl.addWidget(code_header)
        self.code_view = QTextEdit(); self.code_view.setReadOnly(True); self.code_view.setStyleSheet("QTextEdit{background:#1e1e1e;color:#d4d4d4;font-family:'Consolas',monospace;font-size:12px;border:1px solid #333;border-radius:4px;}"); self.code_view.setMinimumHeight(60); bl.addWidget(self.code_view)
        self.main_split = QSplitter(Qt.Vertical); self.main_split.addWidget(top_split); self.main_split.addWidget(bottom); self.main_split.setSizes([600, 300]); root.addWidget(self.main_split)
        self.statusBar().showMessage("就绪 | G键切换网格 | 右键→信号/槽 | Ctrl+Z/Y"); self.statusBar().setStyleSheet("QStatusBar { font-size: 11px; color: #666; border-top: 1px solid #e0e0e0; }")

    def _add_tb_btn2(self, toolbar, text, style=""):
        btn = QToolButton(); btn.setText(text); btn.setToolButtonStyle(Qt.ToolButtonTextOnly)
        if style: btn.setStyleSheet(style)
        fm = btn.fontMetrics(); btn.setMinimumWidth(fm.horizontalAdvance(text) + 16); toolbar.addWidget(btn); return btn

    def _auto_load_custom_widgets(self):
        settings = QSettings("MiniDesigner", "v19"); saved_paths = settings.value("custom_widget_paths", [])
        if saved_paths and isinstance(saved_paths, list):
            valid_paths = [p for p in saved_paths if os.path.isfile(p)]
            if valid_paths:
                count = CustomWidgetLoader.register_widgets(valid_paths); self._custom_widget_files = valid_paths; self.toolbox.populate()
                self.statusBar().showMessage(f"✅ 自动加载 {count} 个自定义控件")
    ...
    def _restore_state(self):
        ...
        if ts:
            ...
            if isinstance(t, QSplitter): t.restoreState(ts)
        dark = settings.value("dark_mode", False)
        if isinstance(dark, str): dark = dark.lower() == "true"
        if dark: self.btn_dark.setChecked(True); self._toggle_dark_mode(True)

    def closeEvent(self, event):
        settings = QSettings("MiniDesigner", "v19"); settings.setValue("window_geometry", self.saveGeometry()); settings.setValue("dark_mode", DARK_MODE)
        settings.setValue("main_split_sizes", self.main_split.saveState()); t = self.main_split.widget(0)
        if isinstance(t, QSplitter): settings.setValue("top_split_sizes", t.saveState())
        settings.setValue("custom_widget_paths", self._custom_widget_files); super().closeEvent(event)

    def _wire(self):
        def on_selection_changed(widget):
            if len(self.canvas._multi_selection) >= 2: self.props.set_batch_widgets(self.canvas._multi_selection)
            else: self.props.set_widget(widget)
        self.canvas.selection_changed.connect(on_selection_changed); self.canvas.widget_modified.connect(self._refresh_code)
        self.canvas.widget_modified.connect(self.props.refresh); self.canvas.widget_modified.connect(self._update_undo_redo_state)
        self.canvas.widget_modified.connect(self.history_panel.refresh); self.btn_undo.clicked.connect(self.canvas.undo); self.btn_redo.clicked.connect(self.canvas.redo)
        self.props.property_changed.connect(self._refresh_code); self.props.property_changed.connect(lambda: (self.canvas._update_handles(), self.canvas.update()))
        self.btn_copy.clicked.connect(self._copy_code); self.btn_clear.clicked.connect(self._clear)
        self.btn_export_qss.clicked.connect(self._export_qss); self.btn_export_project.clicked.connect(self._export_project)
        self.btn_save.clicked.connect(self._save_project); self.btn_load.clicked.connect(self._load_project)
        self.btn_ui_class.toggled.connect(self._toggle_ui_class_mode); self.btn_fixed.toggled.connect(self._toggle_fixed_canvas)
        self.btn_custom.clicked.connect(self._load_custom_widgets); self.btn_theme.clicked.connect(self._open_theme_editor)
        self.btn_template.clicked.connect(self._show_template_menu); self.btn_preview.toggled.connect(self._toggle_preview)
        self.btn_canvas_size.clicked.connect(self._show_canvas_size_menu); self.btn_dark.toggled.connect(self._toggle_dark_mode)
        self.canvas.widget_modified.connect(self.widget_tree.rebuild); self.canvas.widget_modified.connect(self._sync_page_tabs)
        self.widget_tree.item_selected.connect(lambda w: self.props.set_widget(w)); self.widget_tree.rebuild(); self._sync_page_tabs(); self._refresh_code()

    def _toggle_grid_action(self, checked):
        self.canvas._grid_enabled = checked; self.grid_action.setText("📐 网格:ON" if checked else "📐 网格:OFF"); self.canvas.update()

    def _show_canvas_size_menu(self):
        menu = QMenu(self); presets = [("📱 800×600    (SVGA)", 800, 600), ("💻 1024×768  (XGA)", 1024, 768), ("🖥️ 1280×720  (HD)", 1280, 720), ("📺 1366×768  (笔记本)", 1366, 768), ("🖵 1440×900  (WXGA+)", 1440, 900), ("🖳 1600×900  (HD+)", 1600, 900), ("📐 1920×1080 (Full HD)", 1920, 1080), ("🖼️ 2560×1440 (2K)", 2560, 1440), ("📊 3840×2160 (4K)", 3840, 2160)]
        for label, w, h in presets:
            act = menu.addAction(label); act.triggered.connect(lambda checked, ww=w, hh=h, ll=label: self._set_canvas_size(ww, hh, ll))
        menu.addSeparator(); act_custom = menu.addAction("✏️ 自定义尺寸..."); act_custom.triggered.connect(self._custom_canvas_size_dialog)
        btn_rect = self.btn_canvas_size.rect(); global_pos = self.btn_canvas_size.mapToGlobal(QPoint(0, btn_rect.height())); menu.exec(global_pos)

    def _set_canvas_size(self, w, h, label=""):
        self.canvas.set_canvas_size(w, h); self.btn_canvas_size.setText(f"📐 {w}×{h}"); self.statusBar().showMessage(f"✅ 画布尺寸: {w}×{h}")

    def _custom_canvas_size_dialog(self):
        dlg = QDialog(self); dlg.setWindowTitle("自定义画布尺寸"); dlg.resize(300, 150); ly = QVBoxLayout(dlg); form = QHBoxLayout()
        form.addWidget(QLabel("宽度:")); w_spin = QSpinBox(); w_spin.setRange(200, 10000); w_spin.setValue(self.canvas.design_width); form.addWidget(w_spin)
        form.addWidget(QLabel("高度:")); h_spin = QSpinBox(); h_spin.setRange(200, 10000); h_spin.setValue(self.canvas.design_height); form.addWidget(h_spin); ly.addLayout(form)
        btn_layout = QHBoxLayout(); btn_ok = QPushButton("确定"); btn_cancel = QPushButton("取消"); btn_layout.addStretch(); btn_layout.addWidget(btn_ok); btn_layout.addWidget(btn_cancel); ly.addLayout(btn_layout)
        btn_ok.clicked.connect(dlg.accept); btn_cancel.clicked.connect(dlg.reject)
        if dlg.exec() == QDialog.Accepted: self._set_canvas_size(w_spin.value(), h_spin.value())

    def _toggle_dark_mode(self, checked):
        global ACTIVE_QSS, DARK_MODE; DARK_MODE = checked; ACTIVE_QSS = DARK_QSS if checked else DEFAULT_QSS
        QApplication.instance().setStyleSheet(ACTIVE_QSS); self.btn_dark.setText("☀️ 亮色" if checked else "🌙 暗色")
        QSettings("MiniDesigner", "v19").setValue("dark_mode", checked)

    def _toggle_preview(self, checked):
        if checked: self.canvas.enter_preview_mode(); self.btn_preview.setText("✕ 退出预览")
        else: self.canvas.exit_preview_mode(); self.btn_preview.setText("🔍 预览")

    def _on_page_tab_changed(self, idx):
        if idx >= 0 and idx < self.canvas.page_count(): self.canvas.switch_page(idx); self._refresh_code()

    def _on_page_close(self, idx):
        if self.canvas.page_count() <= 1: self.statusBar().showMessage("⚠️ 至少保留一个页面"); return
        self.canvas.remove_page(idx); self._sync_page_tabs(); self._refresh_code()

    def _on_page_rename(self, idx):
        if idx < 0 or idx >= self.canvas.page_count(): return
        old_name = self.canvas.page_name(idx); new_name, ok = QInputDialog.getText(self, "重命名页面", "页面名称:", text=old_name)
        if ok and new_name.strip(): self.canvas.rename_page(idx, new_name.strip()); self._sync_page_tabs(); self._refresh_code()

    def _on_add_page(self):
        idx = self.canvas.add_page(); self._sync_page_tabs(); self.page_tabs.setCurrentIndex(idx); self._refresh_code()
        self.statusBar().showMessage(f"✅ 新建页面: {self.canvas.page_name(idx)}")

    def _sync_page_tabs(self):
        self.page_tabs.blockSignals(True)
        while self.page_tabs.count() > 0: self.page_tabs.removeTab(0)
        for i in range(self.canvas.page_count()): self.page_tabs.addTab(self.canvas.page_name(i))
        self.page_tabs.setCurrentIndex(self.canvas.current_page()); self.page_tabs.blockSignals(False)

    def _new_project(self):
        if self.canvas._canvas_widgets:
            r = QMessageBox.question(self, "新建项目", "当前画布有未保存的内容，是否清空？", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
            if r != QMessageBox.Yes: return
        self.canvas.clear_canvas(); self.canvas.history.undo_stack.clear(); self.canvas.history.redo_stack.clear(); self._current_file = None
        self._sync_page_tabs(); self.setWindowTitle("Mini Designer v20 — 新建项目"); self._refresh_code(); self.statusBar().showMessage("✅ 已新建空白项目")

    def _show_template_menu(self):
        menu = QMenu(self)
        for name, tmpl in INDUSTRIAL_TEMPLATES.items():
            action = menu.addAction(name); action.setToolTip(tmpl["description"]); action.triggered.connect(lambda checked, n=name: self.canvas.insert_template(n))
        user_tmpl = _load_user_templates()
        if user_tmpl:
            menu.addSeparator()
            for name, tmpl in user_tmpl.items():
                a = menu.addAction(f"⭐ {name}"); a.setToolTip(tmpl["description"]); a.triggered.connect(lambda checked, n=name, c=self.canvas: c.insert_template(n))
            menu.addSeparator(); act_manage = menu.addAction("📂 管理用户模板"); act_manage.triggered.connect(self._manage_user_templates)
        else:
            menu.addSeparator(); act_manage = menu.addAction("📂 管理用户模板（无模板）"); act_manage.setEnabled(False)
        menu.addSeparator(); act_import = menu.addAction("📥 从项目文件导入模板"); act_import.triggered.connect(self._import_template_from_project)
        btn_rect = self.btn_template.rect(); global_pos = self.btn_template.mapToGlobal(QPoint(0, btn_rect.height())); menu.exec(global_pos)
    ...
    def _import_template_from_project(self):
        ...
        try:
            ...
            tmpl_widgets = []
            if isinstance(data, list): tmpl_widgets = data
            else:
                for p in data.get("pages", []): tmpl_widgets.extend(p.get("widgets", []))
            if not tmpl_widgets: QMessageBox.warning(self, "导入失败", "项目文件中没有控件"); return
            min_x = min(w.get("x", 0) for w in tmpl_widgets); min_y = min(w.get("y", 0) for w in tmpl_widgets)
            for w in tmpl_widgets: w["x"] = w.get("x", 0) - min_x; w["y"] = w.get("y", 0) - min_y
            name = os.path.splitext(os.path.basename(path))[0]; new_name, ok = QInputDialog.getText(self, "模板名称", "模板名称:", text=name)
            if not ok or not new_name.strip(): return
            USER_TEMPLATES[new_name] = {"description": f"从项目导入: {new_name}", "widgets": tmpl_widgets}; _save_user_templates()
            self.statusBar().showMessage(f"✅ 已从项目导入模板: {new_name}（{len(tmpl_widgets)}个控件）")
        except Exception as e: QMessageBox.warning(self, "导入失败", str(e))

    def _manage_user_templates(self):
        user_tmpl = _load_user_templates(); dlg = QDialog(self); dlg.setWindowTitle("📂 管理用户模板"); dlg.resize(350, 300); layout = QVBoxLayout(dlg)
        list_widget = QListWidget()
        for name in user_tmpl: item = QListWidgetItem(f"⭐ {name}"); item.setData(Qt.UserRole, name); list_widget.addItem(item)
        layout.addWidget(list_widget); btn_row = QHBoxLayout(); btn_delete = QPushButton("🗑️ 删除选中"); btn_delete.setStyleSheet("QPushButton{background:#E74C3C;color:#fff;}")
        btn_close = QPushButton("✅ 关闭"); btn_row.addStretch(); btn_row.addWidget(btn_delete); btn_row.addWidget(btn_close); layout.addLayout(btn_row)
        def do_delete():
            item = list_widget.currentItem()
            if not item: return
            name = item.data(Qt.UserRole)
            if name in user_tmpl:
                del user_tmpl[name]; global USER_TEMPLATES; USER_TEMPLATES.clear(); USER_TEMPLATES.update(user_tmpl); _save_user_templates(); list_widget.takeItem(list_widget.row(item))
        btn_delete.clicked.connect(do_delete); btn_close.clicked.connect(dlg.accept); dlg.exec()

    def _load_custom_widgets(self):
        dlg = QDialog(self); dlg.setWindowTitle("🧩 自定义控件文件管理"); dlg.resize(520, 320); layout = QVBoxLayout(dlg); layout.setContentsMargins(8, 6, 8, 6); layout.setSpacing(4)
        btn_row = QHBoxLayout(); btn_row.setSpacing(6)
        btn_add = QPushButton("📂 添加文件"); btn_add.setToolTip("选择要加载的自定义控件 Python 文件 (.py)"); btn_add.setStyleSheet("QPushButton{background:#4A90D9;color:#fff;border:none;border-radius:4px;padding:5px 12px;font-size:12px;font-weight:bold;}QPushButton:hover{background:#357ABD;}")
        btn_remove = QPushButton("🗑 移除文件"); btn_remove.setEnabled(False); btn_remove.setToolTip("从列表中移除选中的文件"); btn_remove.setStyleSheet("QPushButton{background:#E74C3C;color:#fff;border:none;border-radius:4px;padding:5px 12px;font-size:12px;font-weight:bold;}QPushButton:hover{background:#C0392B;}QPushButton:disabled{background:#ccc;color:#999;}")
        btn_refresh = QPushButton("🔄 刷新"); btn_refresh.setToolTip("重新加载所有文件"); btn_refresh.setStyleSheet("QPushButton{background:#f0f0f0;border:1px solid #ccc;border-radius:4px;padding:5px 12px;font-size:12px;}QPushButton:hover{background:#e0e0e0;}")
        btn_close = QPushButton("✅ 完成"); btn_close.setStyleSheet("QPushButton{background:#27AE60;color:#fff;border:none;border-radius:4px;padding:5px 16px;font-size:12px;font-weight:bold;}QPushButton:hover{background:#219A52;}")
        btn_svg = QPushButton("🎨 导入 SVG"); btn_svg.setToolTip("导入 SVG 矢量图作为控件"); btn_svg.setStyleSheet("QPushButton{background:#8E44AD;color:#fff;border:none;border-radius:4px;padding:5px 12px;font-size:12px;font-weight:bold;}QPushButton:hover{background:#7D3C98;}")
        btn_row.addWidget(btn_add); btn_row.addWidget(btn_remove); btn_row.addWidget(btn_refresh); btn_row.addWidget(btn_svg); btn_row.addStretch(); btn_row.addWidget(btn_close); layout.addLayout(btn_row)
        info_label = QLabel("每个文件独立显示，选中后点击「🗑 移除文件」即可退出"); info_label.setStyleSheet("color:#888; font-size:11px; padding:2px 2px;"); layout.addWidget(info_label)
        file_list = QListWidget(); file_list.setAlternatingRowColors(True); file_list.setStyleSheet("QListWidget{font-size:12px;border:1px solid #ddd;border-radius:4px;}QListWidget::item{padding:6px 8px;}QListWidget::item:selected{background:#4A90D9;color:#fff;}QListWidget::item:alternate{background:#f9f9f9;}")
        for fp in self._custom_widget_files: item = QListWidgetItem(os.path.basename(fp)); item.setData(Qt.UserRole, fp); item.setToolTip(fp); file_list.addItem(item)
        layout.addWidget(file_list, 1); widget_info_label = QLabel(); widget_info_label.setWordWrap(True); widget_info_label.setStyleSheet("color:#666; font-size:11px; padding:4px 6px; background:#f5f5f5; border:1px solid #e0e0e0; border-radius:4px;"); widget_info_label.setMaximumHeight(80); layout.addWidget(widget_info_label)
        def _update_widget_info():
            if not CUSTOM_WIDGETS: widget_info_label.setText("💡 当前无自定义控件，点击「📂 添加文件」加载 .py 文件"); return
            lines = []
            for name, cls, kwargs, fp in CUSTOM_WIDGETS: lines.append(f"  • {name}  ({cls.__name__})  ← {os.path.basename(fp)}")
            widget_info_label.setText("📦 已注册控件:\n" + "\n".join(lines))
        _update_widget_info()
        def _on_selection_changed(): btn_remove.setEnabled(bool(file_list.currentItem()))
        file_list.currentItemChanged.connect(lambda: _on_selection_changed())
        def _refresh_file_list():
            file_list.clear()
            for fp in self._custom_widget_files: item = QListWidgetItem(os.path.basename(fp)); item.setData(Qt.UserRole, fp); item.setToolTip(fp); file_list.addItem(item)
            try:
                from svg_import import SVG_LIBRARY
                for name in SVG_LIBRARY: item = QListWidgetItem(f"🎨 {name}.svg"); item.setData(Qt.UserRole, f"svg:{name}"); item.setToolTip(f"SVG 图元: {name}"); file_list.addItem(item)
            except: pass
            _update_widget_info()
        def _reload_custom_widgets():
            count = CustomWidgetLoader.register_widgets(self._custom_widget_files); self.toolbox.populate()
            settings = QSettings("MiniDesigner", "v19"); settings.setValue("custom_widget_paths", self._custom_widget_files)
            self.statusBar().showMessage(f"✅ 已加载 {count} 个自定义控件（{len(self._custom_widget_files)} 个文件）"); _refresh_file_list()
        def _add_files():
            paths, _ = QFileDialog.getOpenFileNames(dlg, "选择自定义控件Python文件", "", "Python Files (*.py)")
            if not paths: return
            all_paths = list(dict.fromkeys(self._custom_widget_files + paths))
            if all_paths != self._custom_widget_files: self._custom_widget_files = all_paths; _reload_custom_widgets()
        def _import_svg():
            path, _ = QFileDialog.getOpenFileName(dlg, "选择 SVG 文件", "", "SVG Files (*.svg)")
            if not path: return
            try:
                from svg_import import import_svg_file; name, cls = import_svg_file(path); _refresh_file_list(); self.toolbox.populate(); self.statusBar().showMessage(f"✅ 已导入 SVG: {name}")
            except Exception as e: QMessageBox.warning(dlg, "导入失败", str(e))
        def _remove_selected():
            current = file_list.currentItem()
            ...
            if not data: return
            if isinstance(data, str) and data.startswith("svg:"):
                svg_name = data[4:]; reply = QMessageBox.question(dlg, "确认移除", f"确定要移除 SVG 图元「{svg_name}」？", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
                if reply != QMessageBox.Yes: return
                try:
                    from svg_import import SVG_LIBRARY, save_svg_library
                    if svg_name in SVG_LIBRARY: del SVG_LIBRARY[svg_name]; save_svg_library(); self.toolbox.populate(); _refresh_file_list(); self.statusBar().showMessage(f"✅ 已移除 SVG: {svg_name}")
                except: pass
            else:
                fp = data; base_name = os.path.basename(fp); reply = QMessageBox.question(dlg, "确认移除", f"确定要移除文件「{base_name}」及其所有自定义控件？\n{fp}", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
                if reply != QMessageBox.Yes: return
                self._custom_widget_files.remove(fp); _reload_custom_widgets(); _refresh_file_list(); self.statusBar().showMessage(f"✅ 已移除文件: {base_name}")
        btn_add.clicked.connect(_add_files); btn_remove.clicked.connect(_remove_selected); btn_refresh.clicked.connect(lambda: (_reload_custom_widgets(), _refresh_file_list()))
        btn_svg.clicked.connect(_import_svg); btn_close.clicked.connect(dlg.accept); _refresh_file_list(); dlg.exec()

    def _open_theme_editor(self):
        global ACTIVE_QSS; dlg = ThemeEditorDialog(ACTIVE_QSS, self); dlg.qss_changed.connect(self._live_preview_qss)
        result = dlg.exec()
        if result == QDialog.Accepted: ACTIVE_QSS = dlg.get_qss(); QApplication.instance().setStyleSheet(ACTIVE_QSS); self.statusBar().showMessage("✅ 主题已应用")
        else: QApplication.instance().setStyleSheet(ACTIVE_QSS); self.statusBar().showMessage("❌ 主题编辑已取消，已恢复")
    ...
    def _toggle_ui_class_mode(self, checked):
        self._ui_class_mode = checked
        if checked:
            self.btn_ui_class.setText("📦 Ui类模式"); self.btn_ui_class.setStyleSheet("QToolButton{background:#4A90D9;color:#fff;border:none;padding:5px 10px;font-size:12px;font-weight:bold;border-radius:4px;} QToolButton:hover{background:#357ABD;}")
            self.statusBar().showMessage("✅ 当前模式: Ui类模式 — 生成 class Ui_MainWindow(object)")
        else:
            self.btn_ui_class.setText("🪟 独立窗口"); self.btn_ui_class.setStyleSheet("QToolButton{padding:5px 10px;font-size:12px;}")
            self.statusBar().showMessage("✅ 当前模式: 独立窗口 — 生成 class GeneratedWindow(QMainWindow)")
        self._refresh_code()

    def _update_undo_redo_state(self):
        self.btn_undo.setEnabled(self.canvas.history.can_undo()); self.btn_redo.setEnabled(self.canvas.history.can_redo())
    ...
    def _toggle_fixed_canvas(self, checked):
        self.canvas._fixed_canvas = checked
        if checked:
            self.btn_fixed.setText("🔒 固定画布"); self.btn_fixed.setStyleSheet("QToolButton{background:#E67E22;color:#fff;border:none;padding:5px 10px;font-size:12px;font-weight:bold;border-radius:4px;} QToolButton:hover{background:#D68910;}")
            self.statusBar().showMessage("✅ 画布已固定 — 生成 setFixedSize()")
        else:
            self.btn_fixed.setText("🔓 可缩放"); self.btn_fixed.setStyleSheet("QToolButton{padding:5px 10px;font-size:12px;}")
            self.statusBar().showMessage("✅ 画布可缩放 — 生成 resize()")
    ...
    def _export_qss(self):
        global ACTIVE_QSS; path, _ = QFileDialog.getSaveFileName(self, "导出全局样式表", "style.qss", "QSS Files (*.qss)")
        if path:
            with open(path, "w", encoding="utf-8") as f: f.write(ACTIVE_QSS)
            QMessageBox.information(self, "导出成功", f"样式表已保存至:\n{path}")

    def _export_project(self):
        import shutil, traceback; project_name, ok = QInputDialog.getText(self, "导出项目", "请输入项目名称（用作文件名）:", text=os.path.splitext(os.path.basename(self._current_file or ""))[0] or "my_project")
        if not ok or not project_name.strip(): return
        project_name = _sanitize(project_name) or "my_project"; mode_choices = ["独立窗口", "Ui类模式"]
        export_mode, ok = QInputDialog.getItem(self, "导出模式", "选择生成代码的格式:", mode_choices, 0, False)
        if not ok: return
        use_ui_class = (export_mode == "Ui类模式"); target_dir = QFileDialog.getExistingDirectory(self, "选择导出目标目录", "", QFileDialog.ShowDirsOnly)
        if not target_dir: return
        try:
            self.canvas._save_current_page(); code = CodeGenerator.generate(self.canvas, as_ui_class=use_ui_class, qss_filename=f"{project_name}.qss")
            if not code: raise RuntimeError("代码生成为空")
            py_path = os.path.join(target_dir, f"{project_name}.py")
            with open(py_path, "w", encoding="utf-8") as f: f.write(code + "\n")
            reqs = ["PySide6>=6.5.0"]; copied = []
            for _, _, _, filepath in CUSTOM_WIDGETS:
                if not filepath or not os.path.isfile(filepath): continue
                basename = os.path.basename(filepath); dest = os.path.join(target_dir, basename)
                if not os.path.exists(dest): shutil.copy2(filepath, dest); copied.append(basename); reqs.append(os.path.splitext(basename)[0])
            with open(os.path.join(target_dir, "requirements.txt"), "w", encoding="utf-8") as f: f.write("\n".join(reqs) + "\n")
            json_path = os.path.join(target_dir, f"{project_name}.json"); self.canvas.save_project(json_path)
            qss = ACTIVE_QSS if not DARK_MODE else DEFAULT_QSS; qss_path = os.path.join(target_dir, f"{project_name}.qss")
            with open(qss_path, "w", encoding="utf-8") as f: f.write(qss)
            tags_found = any(w.property("_tag") for p in self.canvas._pages for w in p.get("widgets", []) if not w.property("_designer_hidden"))
            if tags_found:
                with open(os.path.join(target_dir, "opc_comm.py"), "w", encoding="utf-8") as f: f.write("import time, threading, random, math\nclass DataSource(threading.Thread):\n    def __init__(self, data_binder):\n        super().__init__(daemon=True)\n        self.binder = data_binder\n        self.running = True\n    def run(self):\n        t = 0\n        while self.running:\n            temp = 75 + 15 * math.sin(t * 0.5)\n            self.binder.update_tag('temp_value', temp)\n            time.sleep(0.5); t += 0.5\n")
            self.statusBar().showMessage(f"✅ 已导出 {project_name} 到: {target_dir}")
            QTimer.singleShot(200, lambda: QMessageBox.information(self, "导出成功", f"项目: {project_name}\n目录: {target_dir}\n{project_name}.py + {project_name}.json + {project_name}.qss + requirements.txt" + (f"\n自定义控件: {', '.join(copied)}" if copied else "") + ("\nopc_comm.py" if tags_found else "")))
        except Exception as e:
            traceback.print_exc(); QTimer.singleShot(200, lambda: QMessageBox.critical(self, "导出失败", str(e)[:200]))

    def _save_project(self):
        import traceback as _tb
        try:
            default = self._current_file or "untitled.json"; path, _ = QFileDialog.getSaveFileName(self, "保存项目", default, "JSON Files (*.json)")
            if not path: return
            self.canvas._deselect(); self.canvas._save_current_page(); QApplication.processEvents()
            self.canvas.save_project(path); self._current_file = path; self.setWindowTitle(f"Mini Designer v20 — {os.path.basename(path)}"); self.statusBar().showMessage(f"✅ 已保存: {path}")
        except Exception as e: _tb.print_exc(); self.statusBar().showMessage(f"❌ 保存失败: {str(e)[:80]}")

    def _load_project(self):
        path, _ = QFileDialog.getOpenFileName(self, "打开项目", "", "JSON Files (*.json)")
        if path: self.canvas.load_project(path); self._current_file = path; self.setWindowTitle(f"Mini Designer v20 — {os.path.basename(path)}"); self._sync_page_tabs(); self._refresh_code(); self.statusBar().showMessage(f"✅ 已加载: {path}")

    def _init_git(self):
        import subprocess
        try: subprocess.run(["git", "--version"], capture_output=True, check=True)
        except (subprocess.SubprocessError, FileNotFoundError): return
        project_dir = os.getcwd(); git_dir = os.path.join(project_dir, ".git")
        if not os.path.isdir(git_dir):
            try:
                subprocess.run(["git", "init"], cwd=project_dir, capture_output=True, check=True); gitignore = os.path.join(project_dir, ".gitignore")
                if not os.path.exists(gitignore):
                    with open(gitignore, "w", encoding="utf-8") as f: f.write("__pycache__/\n*.pyc\n.DS_Store\n")
                subprocess.run(["git", "add", "."], cwd=project_dir, capture_output=True); subprocess.run(["git", "commit", "-m", "初始化项目"], cwd=project_dir, capture_output=True)
            except: pass
        self._autosave_timer = QTimer(self); self._autosave_timer.timeout.connect(self._autosave); self._autosave_timer.start(300000)

    def _autosave(self):
        if not self._current_file: return
        try:
            self.canvas._save_current_page(); self.canvas.save_project(self._current_file); import subprocess; d = os.path.dirname(self._current_file)
            if os.path.isdir(os.path.join(d, ".git")):
                subprocess.run(["git", "add", "-A"], cwd=d, capture_output=True); subprocess.run(["git", "commit", "--allow-empty", "-m", f"自动保存 {os.path.basename(self._current_file)}"], cwd=d, capture_output=True)
            self.statusBar().showMessage(f"💾 已自动保存 ({os.path.basename(self._current_file)})")
        except: pass
