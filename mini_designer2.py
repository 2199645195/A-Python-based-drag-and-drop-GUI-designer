...
class MultiTrendWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        for ch in visible_chs:
            ...
            # 数据不足时从左侧空白开始，最新点始终在最右侧
            x_start_offset = (self._x_range - visible_n) * step_x

            path = QPainterPath()
            for j, val in enumerate(visible_data):
                px = ml + x_start_offset + j * step_x
        ...
        # 绘制 X 轴时间标签（滚动：最右 = 当前时刻，往左递减）
        # 每帧 50ms，_tick_count 累计总帧数
        ...
        for i in range(self._grid_density_x + 1):
            # 当前格对应的绝对时间 = 当前时刻 - (从右往左的偏移)
            t_sec = current_time_s - (self._grid_density_x - i) * time_step * timer_interval_s * self._x_range / self._grid_density_x
            # 简化：直接用帧偏移换算秒，让标签跟着滚动
            elapsed_frames = self._tick_count
            frame_offset = (self._grid_density_x - i) * (self._x_range / self._grid_density_x)
            t_sec = max(0.0, (elapsed_frames - frame_offset) * timer_interval_s)
            gx = ml + pw * i / self._grid_density_x
