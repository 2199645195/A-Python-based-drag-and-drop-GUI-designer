...
包含：Qt 原生缺失控件 + 自创工业/UI控件 + 仪表盘 + 实时曲线
加载方式：mini_designer → 🧩 自定义控件 → 选择本文件
"""
import math, random
...
class GaugeWidget(QWidget):
    ...
    def paintEvent(self, e):
        ...
        ratio = ((self._value - self._min_val) / (self._max_val - self._min_val)
                 ) if self._max_val > self._min_val else 0
...
class CurveWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._grid_color = QColor("#e8e8e8"); self._bg_color = QColor("#ffffff")
        self._tick_count = 0; self._running = False
        ...
        # 预填演示数据，确保创建即有曲线显示
        for _k in range(60):
            self.addValues([50 + 30 * math.sin(_k * 0.2) + random.uniform(-5, 5),
                           30 + 20 * math.cos(_k * 0.25) + random.uniform(-3, 3)])

    def start(self):
        if not self._running: self._running = True; self._demo_timer.start(100)
    def stop(self):
        self._running = False; self._demo_timer.stop()
    ...
    def _demo_tick(self):
        ...
        self._tick_count += 1; self.update()
    ...
    def addValues(self, values):
        for i, v in enumerate(values):
            if i < len(self._channels): self._channels[i]["data"].append(v)
        self._tick_count += 1
        if self._tick_count % 5 == 0: self.update()
    ...
    def setChannelCount(self, n):
        ...
        while len(self._channels) < n:
            ...
            self._channels.append({"name": f"通道{idx + 1}", "color": colors[idx % len(colors)],
                                   "data": deque(maxlen=300)})
        self._channels = self._channels[:max(1, n)]; self.update()
    ...
    def paintEvent(self, e):
        p = QPainter(self); p.setRenderHint(QPainter.Antialiasing)
        w, h = self.width(), self.height()
        ml, mr, mt, mb = 45, 15, 25, 30
        pw, ph = w - ml - mr, h - mt - mb
        if pw <= 0 or ph <= 0: return
        p.fillRect(0, 0, w, h, self._bg_color)
        p.setPen(QPen(self._grid_color, 1))
        for i in range(5):
            y = mt + ph * i / 4.0; p.drawLine(QPointF(ml, y), QPointF(w - mr, y))
        for i in range(6):
            x = ml + pw * i / 5.0; p.drawLine(QPointF(x, mt), QPointF(x, h - mb))
        p.setPen(QPen(QColor("#ccc"), 1.5)); p.drawRect(ml, mt, pw, ph)
        if self._auto_range:
            all_vals = []
            ...
            else: self._y_min, self._y_max = 0.0, 100.0
        yr = self._y_max - self._y_min or 1.0
        p.setFont(QFont("Microsoft YaHei", 8)); p.setPen(QColor("#666"))
        for i in range(5):
            val = self._y_max - yr * i / 4.0; y = mt + ph * i / 4.0
            p.drawText(QRectF(2, y - 8, ml - 6, 16), Qt.AlignRight | Qt.AlignVCenter, f"{val:.1f}")
        for ch in self._channels:
            data = list(ch["data"])
            if len(data) < 2: continue
            pen = QPen(ch["color"], 2); p.setPen(pen); n = len(data)
            for i in range(1, n):
                x1 = ml + pw * (i - 1) / max(1, n - 1)
                x2 = ml + pw * i / max(1, n - 1)
                y1 = mt + ph * (1.0 - (data[i - 1] - self._y_min) / yr)
                y2 = mt + ph * (1.0 - (data[i] - self._y_min) / yr)
                p.drawLine(QPointF(x1, max(mt, min(mt + ph, y1))),
                          QPointF(x2, max(mt, min(mt + ph, y2))))
            if data:
                lx = ml + pw; ly = mt + ph * (1.0 - (data[-1] - self._y_min) / yr)
                ly = max(mt, min(mt + ph, ly))
                p.setBrush(ch["color"]); p.setPen(Qt.NoPen)
                p.drawEllipse(QPointF(lx, ly), 3, 3)
        if self._title:
            p.setFont(QFont("Microsoft YaHei", 9, QFont.Bold)); p.setPen(QColor("#333"))
            p.drawText(QRectF(ml, 2, pw, mt - 2), Qt.AlignCenter, self._title)
        lx, ly = ml + 4, mt + 4
        p.setFont(QFont("Microsoft YaHei", 8))
        for ch in self._channels:
            p.setPen(Qt.NoPen); p.setBrush(ch["color"])
            p.drawRect(int(lx), int(ly), 10, 10)
            p.setPen(QColor("#333"))
            p.drawText(QRectF(lx + 14, ly, 60, 12), Qt.AlignLeft, ch["name"])
            ly += 14
        p.end()
...
class KeySequenceEdit(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._keys = ""; self._recording = False
        self.setStyleSheet("KeySequenceEdit{background:#fff;border:1px solid #d0d0d0;border-radius:4px;padding:4px 8px;font-size:12px;}")
    ...
    def paintEvent(self, e):
        ...
        p.drawText(r, Qt.AlignCenter, "按下快捷键..." if self._recording else (self._keys or "点击设置快捷键"))
        p.end()

    def mousePressEvent(self, e): self._recording = True; self._keys = ""; self.update()
    ...
    def keyPressEvent(self, e):
        ...
        if key not in (Qt.Key_Control, Qt.Key_Shift, Qt.Key_Alt, Qt.Key_Meta, Qt.Key_unknown):
            ...
            self._keys = "+".join(parts); self._recording = False
            self.keyChanged.emit(self._keys); self.update()
...
class FontComboBox(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.combo = QFontComboBox(); self.combo.setMinimumHeight(28)
        ...
        self.sample = QLabel("Aa"); self.sample.setMinimumWidth(36)
...
class HorizontalScrollBar(QScrollBar):
    ...
    def __init__(self, parent=None):
        ...
        self.setRange(0, 100); self.setValue(0); self.setMinimumHeight(18)
...
class VerticalScrollBar(QScrollBar):
    ...
    def __init__(self, parent=None):
        ...
        self.setRange(0, 100); self.setValue(0); self.setMinimumWidth(18)
...
class ToggleSwitch(QWidget):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self._checked = False
        self._on_color = QColor("#27AE60"); self._off_color = QColor("#ccc")
    ...
    def setChecked(self, v):
        ...
        self.toggled.emit(self._checked); self.valueChanged.emit(self._checked); self.update()
    ...
    def paintEvent(self, e):
        ...
        p.setPen(Qt.NoPen); p.setBrush(QColor("#e0e0e0") if not self._checked else track_color.lighter(150))
...
class LedIndicator(QWidget):
    ...
    LED_COLORS = {"red": QColor("#E74C3C"), "green": QColor("#27AE60"), "yellow": QColor("#F39C12"),
                  "blue": QColor("#4A90D9"), "white": QColor("#ecf0f1"), "orange": QColor("#E67E22")}

    def __init__(self, parent=None):
        super().__init__(parent)
        self._color = QColor("#27AE60"); self._on = True; self._blinking = False
        self._blink_timer = QTimer(self)
        self._blink_timer.timeout.connect(lambda: (setattr(self, '_on', not self._on), self.update()))
        self.setFixedSize(28, 28)

    def setColor(self, name): self._color = self.LED_COLORS.get(name, QColor(name)); self.update()
    ...
    def paintEvent(self, e):
        p = QPainter(self); p.setRenderHint(QPainter.Antialiasing)
        s = min(self.width(), self.height()) - 4; cx, cy = self.width() / 2, self.height() / 2; r = s / 2
        glow = QRadialGradient(cx, cy, r * 1.4)
        c = self._color if self._on else QColor("#888")
        glow.setColorAt(0, c.lighter(180)); glow.setColorAt(0.7, c)
...
class CircularProgress(QWidget):
    ...
    def paintEvent(self, e):
        p = QPainter(self); p.setRenderHint(QPainter.Antialiasing)
        w, h = self.width(), self.height()
        side = min(w, h) - self._line_width - 8; cx, cy = w / 2, h / 2; r = side / 2
        ...
        if self._text_visible:
            ...
            p.drawText(QRectF(cx - r, cy - r * 0.3, r * 2, r * 0.6), Qt.AlignCenter, f"{int(ratio * 100)}%")
        p.end()
...
class ColorPickerButton(QPushButton):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self._color = QColor("#4A90D9"); self.setText("选择颜色")
        self.clicked.connect(self._pick); self._update_style()

    def color(self): return self._color
    def setColor(self, c):
        self._color = QColor(c) if isinstance(c, str) else c
        self._update_style(); self.colorChanged.emit(self._color.name())
    ...
    def _update_style(self):
        self.setStyleSheet(f"QPushButton{{background:{self._color.name()};color:#fff;border:none;border-radius:4px;padding:6px 14px;font-size:12px;font-weight:bold;}}QPushButton:hover{{background:{self._color.darker(110).name()};}}")
...
class FilePicker(QWidget):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        layout = QHBoxLayout(self); layout.setContentsMargins(0, 0, 0, 0); layout.setSpacing(4)
        self.edit = QLineEdit(); self.edit.setPlaceholderText("选择文件路径...")
        ...
        self.btn = QPushButton("📂"); self.btn.setFixedWidth(32); self.btn.setMinimumHeight(28)
...
class StarRating(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._star_color = QColor("#F39C12"); self._empty_color = QColor("#ddd")
    ...
    def paintEvent(self, e):
        ...
        for i in range(self._max):
            ...
            self._draw_star(p, i * 32 + 16, 16, 14)
        p.end()
    ...
    def mousePressEvent(self, e):
        ...
        self.setValue(idx + 1); self.ratingChanged.emit(self._rating)
...
class DigitalClock(QWidget):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self._format = "HH:mm:ss"; self._color = QColor("#4A90D9"); self._bg = QColor("#1e1e1e")
        self.setMinimumSize(160, 60)
        self._timer = QTimer(self); self._timer.timeout.connect(self.update); self._timer.start(1000)
    ...
    def paintEvent(self, e):
        ...
        p.setPen(self._color)
        fm = QFontMetrics(QFont("Consolas", max(12, self.height() // 2), QFont.Bold))
...
class IPAddressEdit(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._octets = []
        for i in range(4):
            sb = QSpinBox(); sb.setRange(0, 255); sb.setValue(192 if i == 0 else 168 if i == 1 else 0)
            sb.setMinimumWidth(50); sb.setAlignment(Qt.AlignCenter)
            ...
            layout.addWidget(sb); self._octets.append(sb)
            if i < 3:
                dot = QLabel("."); dot.setStyleSheet("font-weight:bold;font-size:14px;")
                dot.setAlignment(Qt.AlignCenter); dot.setFixedWidth(8)
                layout.addWidget(dot)
        layout.addStretch()

    def ip(self): return ".".join(str(o.value()) for o in self._octets)
    def setIP(self, ip_str):
        parts = ip_str.split(".")
        for i, p in enumerate(parts[:4]):
            try: self._octets[i].setValue(int(p))
            except: pass
    def _on_change(self, idx, val): self.ipChanged.emit(self.ip())
    def setText(self, t): self.setIP(t)
...
class BatteryIndicator(QWidget):
    ...
    def paintEvent(self, e):
        ...
        color = QColor("#27AE60") if ratio > 0.2 else (QColor("#F39C12") if ratio > 0.05 else QColor("#E74C3C"))
...
class TagChip(QWidget):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self._text = "标签"; self._color = QColor("#4A90D9"); self._removable = False
...
class Knob(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._start_angle = 225; self._span_angle = 270
        self._color = QColor("#4A90D9"); self._dragging = False
    ...
    def paintEvent(self, e):
        ...
        p.setPen(pen2)
        p.drawArc(QRectF(cx - r, cy - r, r * 2, r * 2), int(self._start_angle * 16), int(self._span_angle * ratio * 16))
        ang = (self._start_angle + self._span_angle * ratio) * math.pi / 180
        dot_r = r - 8; dx = cx + dot_r * math.cos(ang); dy = cy - dot_r * math.sin(ang)
        ...
        p.drawText(QRectF(cx - r, cy - r * 0.2, r * 2, r * 0.4), Qt.AlignCenter, f"{self._value:.0f}")
        p.end()

    def mousePressEvent(self, e):
        self._dragging = True; self.setCursor(Qt.ClosedHandCursor)
    ...
    def mouseReleaseEvent(self, e):
        self._dragging = False; self.setCursor(Qt.OpenHandCursor)

    def _update_from_mouse(self, pos):
        cx, cy = self.width() / 2, self.height() / 2
        ang = math.atan2(cy - pos.y(), pos.x() - cx) * 180 / math.pi
        a = (ang + 360) % 360
        start = self._start_angle; span = self._span_angle
...
class TimerDisplay(QWidget):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self._total_seconds = 60; self._remaining = 60
        self._running = False; self._mode = "countdown"
        self._timer = QTimer(self); self._timer.timeout.connect(self._tick)
        self.setMinimumSize(120, 50)

    def setValue(self, v): self._total_seconds = int(v); self._remaining = int(v); self.update()
    def value(self): return self._remaining
    def setRange(self, mn, mx): self._total_seconds = mx; self._remaining = mx; self.update()
    def start(self): self._running = True; self._timer.start(1000)
    def stop(self): self._running = False; self._timer.stop()
    def reset(self): self.stop(); self._remaining = self._total_seconds; self.update()

    def _tick(self):
        if self._mode == "countdown":
            self._remaining -= 1
            if self._remaining <= 0: self._remaining = 0; self.stop(); self.timeout.emit()
        else: self._remaining += 1
        self.valueChanged.emit(self._remaining); self.update()
    ...
    def paintEvent(self, e):
        ...
        p.drawRoundedRect(QRectF(self.rect().adjusted(2, 2, -2, -2)), 8, 8)
        mins = abs(self._remaining) // 60; secs = abs(self._remaining) % 60
...
class SegmentDisplay(QWidget):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self._value = "8888"; self._color_on = QColor("#00ff88"); self._color_off = QColor("#0a2a1a")
        self._digit_count = 4; self._decimals = 0
        self.setMinimumSize(120, 50)

    def setValue(self, v):
        try: self._value = f"{float(v):0{self._digit_count}.{self._decimals}f}"
        except (ValueError, TypeError): self._value = str(v)[:self._digit_count]
        self.update()
    ...
    def paintEvent(self, e):
        ...
        w, h = self.width(), self.height()
        dw = w / self._digit_count; dh = h * 0.8
        ox = (w - dw * self._digit_count) / 2; oy = (h - dh) / 2
        for i, ch in enumerate(self._value[:self._digit_count]):
            self._draw_digit(p, ox + i * dw + 2, oy, dw - 4, dh, ch, self._color_on, self._color_off)
        p.end()
    ...
    def _draw_digit(self, p, x, y, w, h, ch, on, off):
        segs = {'0': 'abcdef', '1': 'bc', '2': 'abged', '3': 'abgcd', '4': 'fgbc',
                ...
                '-': 'g', ' ': '', 'A': 'abcefg', 'E': 'afedg', 'F': 'afeg', 'H': 'fegbc'}
        ...
        for name, pts in seg_defs.items():
            p.setBrush(on if name in active else off); p.setPen(Qt.NoPen)
            path = QPainterPath()
...
class ValueCard(QWidget):
    """数值显示卡片 — 标题+数值+单位+趋势箭头"""
    _display_name = "数值卡片"

    def __init__(self, parent=None):
        super().__init__(parent)
        self._title = "参数"; self._value = 0.0; self._unit = ""
        self._decimals = 1; self._trend = 0; self._color = QColor("#4A90D9")
    ...
    def paintEvent(self, e):
        ...
        p.setPen(QColor("#888")); p.setFont(QFont("Microsoft YaHei", 10))
        ...
        p.setPen(QColor("#666")); p.setFont(QFont("Microsoft YaHei", 10))
        p.drawText(QRectF(self.width() - 50, self.height() - 26, 44, 20), Qt.AlignRight, self._unit)
        if self._trend != 0:
            tx, ty = self.width() - 20, 8; p.setPen(Qt.NoPen)
            if self._trend > 0:
