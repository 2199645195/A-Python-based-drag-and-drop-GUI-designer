...
class DesignerMainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        self.project_panel = ProjectPanel(self.canvas, self)
        left_tabs.addTab(self.project_panel, "📂 项目")
        ...
        self.btn_toggle_code = QPushButton("▼ 收起"); self.btn_toggle_code.setFixedWidth(60)
        self.btn_toggle_code.setStyleSheet("QPushButton{padding:2px 6px;font-size:11px;}")
        ...
        code_header_l.addWidget(self.btn_copy); code_header_l.addWidget(self.btn_toggle_code)
    ...
    def _wire(self):
        ...
        self.btn_toggle_code.clicked.connect(self._toggle_code_view)
    ...
    # ── Git 初始化（方便回滚）──
    def _init_git(self):
        """检查/初始化 Git，后续 AI 改代码前会自动 commit"""
        import subprocess
        try:
        ...
        if not os.path.isdir(git_dir):
            try:
                ...
                subprocess.run(["git", "commit", "-m", "初始化项目"], cwd=project_dir, capture_output=True)
            except Exception:
                pass
        except Exception:
            pass
    ...
    def _toggle_code_view(self):
        ...
        self.btn_toggle_code.setText("▶ 展开" if not visible else "▼ 收起")
...
class ProjectPanel(QWidget):
    """左侧项目面板 — 显示文件列表，快速打开/保存"""

    def __init__(self, canvas, main_window, parent=None):
        super().__init__(parent)
        self.canvas = canvas
        self.main = main_window
        layout = QVBoxLayout(self); layout.setContentsMargins(4, 4, 4, 4); layout.setSpacing(4)

        self.header = QLabel("<b>📂 项目文件</b>")
        self.header.setStyleSheet("padding:4px; font-size:12px;")
        layout.addWidget(self.header)

        self.path_label = QLabel("未保存")
        self.path_label.setStyleSheet("color:#888; font-size:10px; padding:2px;")
        layout.addWidget(self.path_label)

        self.file_list = QListWidget()
        self.file_list.setStyleSheet("QListWidget{font-size:11px;}")
        layout.addWidget(self.file_list)

        btn_row = QHBoxLayout()
        self.btn_open = QPushButton("📂 打开"); self.btn_open.setStyleSheet("QPushButton{padding:4px 8px;font-size:11px;}")
        self.btn_save = QPushButton("💾 保存"); self.btn_save.setStyleSheet("QPushButton{padding:4px 8px;font-size:11px;background:#4A90D9;color:#fff;}")
        btn_row.addWidget(self.btn_open); btn_row.addWidget(self.btn_save)
        layout.addLayout(btn_row)

        self.btn_open.clicked.connect(self._on_open)
        self.btn_save.clicked.connect(self._on_save)
        self.canvas.widget_modified.connect(self.refresh)

    def refresh(self):
        self.path_label.setText(self.main._current_file or "未保存")
        self.file_list.clear()
        d = os.path.dirname(self.main._current_file) if self.main._current_file else os.getcwd()
        if not os.path.isdir(d): return
        for f in sorted(os.listdir(d)):
            if f.endswith(('.json', '.py', '.qss', '.txt')):
    ...
    def _on_item_double_click(self, item):
        ...
        if path.endswith('.json'):
            ...
            self.main._current_file = path
            self.main.setWindowTitle(f"Mini Designer — {os.path.basename(path)}")
            self.main._sync_page_tabs()
            self.main._refresh_code()
