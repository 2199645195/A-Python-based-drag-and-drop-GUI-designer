...
# ══════════════════════════════════════════════════════════════
# ══════════════════════════════════════════════════════════════
# P20: 工业级多通道趋势曲线 (内置版 + 工业标准秒级时间轴)
# ══════════════════════════════════════════════════════════════
class MultiTrendWidget(QWidget):
    """工业级多通道实时趋势曲线 (纯正时:分:秒滚动时间轴)"""
    ...
    def start(self):
        if not self._running:
            ...
            if not self._demo_timer.isActive():
                self._demo_timer.start(50) # 50ms 刷新一次
    ...
    def paintEvent(self, event):
        ...
        if self._auto_scale_y and visible_chs:
            ...
            for ch in visible_chs:
                ch_data = list(ch["data"])[-max_visible:]
                visible_vals.extend([v for t, v in ch_data])
            if visible_vals:
                y_min, y_max = min(visible_vals), max(visible_vals)
        ...
        for i in range(self._grid_density_y + 1):
            ...
            gy = mt + ph * i / self._grid_density_y
            p.drawText(2, int(gy) - 6, ml - 6, 12, Qt.AlignRight | Qt.AlignVCenter, f"{val:.1f}")
            
        # ── 绘制动态滚动的纯秒时间轴 ──
        t_first = time.time() - (max_visible * 0.05)
        dt_per_step = 0.05
        ...
        for ch in visible_chs:
            ...
            if len(data) >= 2:
                t_first = data[0][0]
                t_last = data[-1][0]
                dt_per_step = (t_last - t_first) / (len(data) - 1) if t_last != t_first else 0.05
                break
        ...
        for i in range(self._grid_density_x + 1):
            ...
            p.drawLine(int(gx), mt + ph, int(gx), mt + ph + 4) # 刻度线
            
            # 计算当前垂直网格线对应的精确绝对时间戳
            idx_f = (gx - ml) / step_x
            t_grid = t_first + idx_f * dt_per_step
            
            # 【修改核心】使用 int(t_grid) 直接剥离微秒/毫秒，只保留工业整秒依次递增
            lt = time.localtime(int(t_grid))
            label = time.strftime("%H:%M:%S", lt)
            
            # 稳定绘制 X 轴时间标签
            p.setPen(QPen(self._text_color, 1))
            p.drawText(int(gx) - 30, mt + ph + 8, 60, 15, Qt.AlignCenter, label)
